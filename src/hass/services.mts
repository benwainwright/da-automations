// This file is generated by type-writer
// Changes will not be preserved if you run the command again
import "@digital-alchemy/hass";

export const InstalledAddons = {
  "Code Glue (Dev)": "62ac2746_code_glue_dev",
  "Code Runner": "3f718f42_code_runner",
  "ESPHome Device Builder": "5c53de3b_esphome",
  "File editor": "core_configurator",
  "Get HACS": "cb646a50_get",
  Hakit: "5c36e1c9_hakit",
  "Jellyfin Server (Host Network)": "3ce0ef5e_jellyfinserverhn",
  "Matter Server": "core_matter_server",
  "Mosquitto broker": "core_mosquitto",
  "Music Assistant": "d5369777_music_assistant",
  "Music Assistant Alexa Skill": "6fe94cfb_music_assistant_skill",
  "OpenThread Border Router": "core_openthread_border_router",
  Piper: "core_piper",
  "Postgres 17": "db21ed7f_postgres_latest",
  Radarr: "a0d7b954_radarr",
  "SQLite Web": "a0d7b954_sqlite-web",
  "Studio Code Server": "a0d7b954_vscode",
  Whisparr: "a0d7b954_whisparr",
  "Z-Wave JS": "core_zwave_js",
  Zigbee2MQTT: "45df7312_zigbee2mqtt",
} as const;

import {
  AndroidNotificationData,
  AppleNotificationData,
  NotificationData,
  PICK_ENTITY,
  PICK_FROM_PLATFORM,
  SupportedCountries,
  SupportedLanguages,
  TAreaId,
  TDeviceId,
  TLabelId,
  WeatherGetForecasts,
} from "@digital-alchemy/hass";
import { EmptyObject, LiteralUnion, RequireAtLeastOne } from "type-fest";

declare module "@digital-alchemy/hass" {
  export interface iCallService {
    // # MARK: adaptive_lighting
    adaptive_lighting: {
      /**
       * ### apply
       *
       * > Applies the current Adaptive Lighting settings to lights.
       */
      apply: (service_data?: {
        /**
         * ## adapt_brightness
         *
         * > Whether to adapt the brightness of the light. 🌞
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "adapt_brightness": "true"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        adapt_brightness?: boolean;
        /**
         * ## adapt_color
         *
         * > Whether to adapt the color on supporting lights. 🌈
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "adapt_color": "true"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        adapt_color?: boolean;
        /**
         * ## entity_id
         *
         * > The `entity_id` of the switch with the settings to apply. 📝
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   integration: adaptive_lighting
         * >   domain:
         * >     - switch
         * >   multiple: false
         * >   reorder: false
         * > ```
         */
        entity_id?:
          | PICK_FROM_PLATFORM<"adaptive_lighting", "switch">
          | PICK_FROM_PLATFORM<"adaptive_lighting", "switch">[];
        /**
         * ## lights
         *
         * > A light (or list of lights) to apply the settings to. 💡
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - light
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        lights?: PICK_ENTITY<"light"> | PICK_ENTITY<"light">[];
        /**
         * ## prefer_rgb_color
         *
         * > Whether to prefer RGB color adjustment over light color temperature when possible. 🌈
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "prefer_rgb_color": "false"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        prefer_rgb_color?: boolean;
        /**
         * ## transition
         *
         * > Duration of transition when lights change, in seconds. 🕑
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "transition": "10"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        transition?: string;
        /**
         * ## turn_on_lights
         *
         * > Whether to turn on lights that are currently off. 🔆
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "turn_on_lights": "false"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        turn_on_lights?: boolean;
      }) => Promise<void>;
      /**
       * ### change_switch_settings
       *
       * > Change any settings you'd like in the switch. All options here are the same as in the config flow.
       */
      change_switch_settings: (service_data?: {
        /**
         * ## adapt_delay
         *
         * > Wait time (seconds) between light turn on and Adaptive Lighting applying changes. Might help to avoid flickering. ⏲️
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "adapt_delay": "0"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        adapt_delay?: string;
        /**
         * ## autoreset_control_seconds
         *
         * > Automatically reset the manual control after a number of seconds. Set to 0 to disable. ⏲️
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "autoreset_control_seconds": "0"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        autoreset_control_seconds?: string;
        /**
         * ## detect_non_ha_changes
         *
         * > Detects and halts adaptations for non-`light.turn_on` state changes. Needs `take_over_control` enabled. 🕵️ Caution: ⚠️ Some lights might falsely indicate an 'on' state, which could result in lights turning on unexpectedly. Note that this calls `homeassistant.update_entity` every `interval`! Disable this feature if you encounter such issues.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "detect_non_ha_changes": "false"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        detect_non_ha_changes?: boolean;
        /**
         * ## entity_id
         *
         * > Entity ID of the switch. 📝
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - switch
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id: PICK_ENTITY<"switch"> | PICK_ENTITY<"switch">[];
        /**
         * ## include_config_in_attributes
         *
         * > Show all options as attributes on the switch in Home Assistant when set to `true`. 📝
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        include_config_in_attributes?: boolean;
        /**
         * ## initial_transition
         *
         * > Duration of the first transition when lights turn from `off` to `on` in seconds. ⏲️
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "initial_transition": "1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        initial_transition?: string;
        /**
         * ## max_brightness
         *
         * > Maximum brightness percentage. 💡
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "max_brightness": "100"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        max_brightness?: string;
        /**
         * ## max_color_temp
         *
         * > Coldest color temperature in Kelvin. ❄️
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "max_color_temp": "5500"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        max_color_temp?: string;
        /**
         * ## max_sunrise_time
         *
         * > Set the latest virtual sunrise time (HH:MM:SS), allowing for earlier sunrises. 🌅
         *
         * ## Selector
         *
         * > ```yaml
         * > time: {}
         * > ```
         */
        max_sunrise_time?: string;
        /**
         * ## min_brightness
         *
         * > Minimum brightness percentage. 💡
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "min_brightness": "1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        min_brightness?: string;
        /**
         * ## min_color_temp
         *
         * > Warmest color temperature in Kelvin. 🔥
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "min_color_temp": "2000"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        min_color_temp?: string;
        /**
         * ## min_sunset_time
         *
         * > Set the earliest virtual sunset time (HH:MM:SS), allowing for later sunsets. 🌇
         *
         * ## Selector
         *
         * > ```yaml
         * > time: {}
         * > ```
         */
        min_sunset_time?: string;
        /**
         * ## only_once
         *
         * > Adapt lights only when they are turned on (`true`) or keep adapting them (`false`). 🔄
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "only_once": "false"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        only_once?: boolean;
        /**
         * ## prefer_rgb_color
         *
         * > Whether to prefer RGB color adjustment over light color temperature when possible. 🌈
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "prefer_rgb_color": "false"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        prefer_rgb_color?: boolean;
        /**
         * ## send_split_delay
         *
         * > Delay (ms) between `separate_turn_on_commands` for lights that don't support simultaneous brightness and color setting. ⏲️
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "send_split_delay": "0"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        send_split_delay?: boolean;
        /**
         * ## separate_turn_on_commands
         *
         * > Use separate `light.turn_on` calls for color and brightness, needed for some light types. 🔀
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "separate_turn_on_commands": "false"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        separate_turn_on_commands?: boolean;
        /**
         * ## sleep_brightness
         *
         * > Brightness percentage of lights in sleep mode. 😴
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "sleep_brightness": "1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        sleep_brightness?: string;
        /**
         * ## sleep_color_temp
         *
         * > Color temperature in sleep mode (used when `sleep_rgb_or_color_temp` is `color_temp`) in Kelvin. 😴
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "sleep_color_temp": "1000"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        sleep_color_temp?: string;
        /**
         * ## sleep_rgb_color
         *
         * > RGB color in sleep mode (used when `sleep_rgb_or_color_temp` is "rgb_color"). 🌈
         *
         * ## Selector
         *
         * > ```yaml
         * > color_rgb: {}
         * > ```
         */
        sleep_rgb_color?: [number, number, number];
        /**
         * ## sleep_rgb_or_color_temp
         *
         * > Use either `"rgb_color"` or `"color_temp"` in sleep mode. 🌙
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "sleep_rgb_or_color_temp": "color_temp"
         * > }
         * > ```
         */
        sleep_rgb_or_color_temp?: "rgb_color" | "color_temp";
        /**
         * ## sleep_transition
         *
         * > Duration of transition when "sleep mode" is toggled in seconds. 😴
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "sleep_transition": "1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        sleep_transition?: string;
        /**
         * ## sunrise_offset
         *
         * > Adjust sunrise time with a positive or negative offset in seconds. ⏰
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "sunrise_offset": "0"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 0
         * >   max: 86300
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        sunrise_offset?: number;
        /**
         * ## sunrise_time
         *
         * > Set a fixed time (HH:MM:SS) for sunrise. 🌅
         *
         * ## Selector
         *
         * > ```yaml
         * > time: {}
         * > ```
         */
        sunrise_time?: string;
        /**
         * ## sunset_offset
         *
         * > Adjust sunset time with a positive or negative offset in seconds. ⏰
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 0
         * >   max: 86300
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        sunset_offset?: number;
        /**
         * ## sunset_time
         *
         * > Set a fixed time (HH:MM:SS) for sunset. 🌇
         *
         * ## Selector
         *
         * > ```yaml
         * > time: {}
         * > ```
         */
        sunset_time?: string;
        /**
         * ## take_over_control
         *
         * > Pause adaptation of individual lights and hand over (manual) control to other sources that issue `light.turn_on` calls for lights that are on. 🔒
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "take_over_control": "true"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        take_over_control?: boolean;
        /**
         * ## take_over_control_mode
         *
         * > The adaptation pausing mode when other sources change brightness and/or color of lights. `pause_all` always pauses both brightness and color adaptation. `pause_changed` pauses the adaptation of only the changed attributes and continues adapting unchanged attributes, e.g., continues color adaptation when only brightness was changed.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "take_over_control_mode": "pause_changed"
         * > }
         * > ```
         */
        take_over_control_mode?: "pause_all" | "pause_changed";
        /**
         * ## transition
         *
         * > Duration of transition when lights change, in seconds. 🕑
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "transition": "45"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        transition?: string;
        /**
         * ## turn_on_lights
         *
         * > Whether to turn on lights that are currently off. 🔆
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "turn_on_lights": "false"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        turn_on_lights?: boolean;
        /**
         * ## use_defaults
         *
         * > Sets the default values not specified in this service call. Options: "current" (default, retains current values), "factory" (resets to documented defaults), or "configuration" (reverts to switch config defaults). ⚙️
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "use_defaults": "current"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > "current"
         * > ```
         */
        use_defaults?: "current" | "configuration" | "factory";
      }) => Promise<void>;
      /**
       * ### set_manual_control
       *
       * > Mark whether a light is 'manually controlled'.
       */
      set_manual_control: (service_data?: {
        /**
         * ## entity_id
         *
         * > The `entity_id` of the switch in which to (un)mark the light as being `manually controlled`. 📝
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   integration: adaptive_lighting
         * >   domain:
         * >     - switch
         * >   multiple: false
         * >   reorder: false
         * > ```
         */
        entity_id?:
          | PICK_FROM_PLATFORM<"adaptive_lighting", "switch">
          | PICK_FROM_PLATFORM<"adaptive_lighting", "switch">[];
        /**
         * ## lights
         *
         * > entity_id(s) of lights, if not specified, all lights in the switch are selected. 💡
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - light
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        lights?: PICK_ENTITY<"light"> | PICK_ENTITY<"light">[];
        /**
         * ## manual_control
         *
         * > Whether to add ("true") or remove ("false") all adapted attributes of the light from the "manual_control" list, or the name of an attribute for selective addition. 🔒
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "manual_control": "true"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > true
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        manual_control?: boolean;
      }) => Promise<void>;
    };
    // # MARK: ai_task
    ai_task: {
      /**
       * ### generate_data
       *
       * >
       */
      generate_data: <T = unknown>(service_data?: {
        /**
         * ## attachments
         *
         * ## Selector
         *
         * > ```yaml
         * > media:
         * >   accept:
         * >     - '*'
         * >   multiple: true
         * > ```
         */
        attachments?: string;
        /**
         * ## entity_id
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   filter:
         * >     - domain:
         * >         - ai_task
         * >       supported_features:
         * >         - 1
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id?: PICK_ENTITY | PICK_ENTITY[];
        /**
         * ## instructions
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "instructions": "Generate a funny notification that the garage door was left open"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiline: true
         * >   multiple: false
         * > ```
         */
        instructions: string;
        /**
         * ## structure
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "structure": "{ \"name\": { \"selector\": { \"text\": }, \"description\": \"Name of the user\", \"required\": \"True\" } } }, \"age\": { \"selector\": { \"number\": }, \"description\": \"Age of the user\" } }"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        structure?: Record<string, unknown> | unknown[];
        /**
         * ## task_name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "task_name": "home summary"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        task_name: string;
      }) => Promise<T>;
      /**
       * ### generate_image
       *
       * >
       */
      generate_image: <T = unknown>(service_data?: {
        /**
         * ## attachments
         *
         * ## Selector
         *
         * > ```yaml
         * > media:
         * >   accept:
         * >     - '*'
         * >   multiple: true
         * > ```
         */
        attachments?: string;
        /**
         * ## entity_id
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   filter:
         * >     - domain:
         * >         - ai_task
         * >       supported_features:
         * >         - 4
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id: PICK_ENTITY | PICK_ENTITY[];
        /**
         * ## instructions
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "instructions": "Generate a high quality square image of a dog on transparent background"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiline: true
         * >   multiple: false
         * > ```
         */
        instructions: string;
        /**
         * ## task_name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "task_name": "picture of a dog"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        task_name: string;
      }) => Promise<T>;
    };
    // # MARK: alarm_control_panel
    alarm_control_panel: {
      /**
       * ### alarm_arm_away
       *
       * >
       */
      alarm_arm_away: (
        service_data?: {
          /**
           * ## code
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "code": "1234"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          code?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - alarm_control_panel
           * >     supported_features:
           * >       - 2
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### alarm_arm_custom_bypass
       *
       * >
       */
      alarm_arm_custom_bypass: (
        service_data?: {
          /**
           * ## code
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "code": "1234"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          code?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - alarm_control_panel
           * >     supported_features:
           * >       - 16
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### alarm_arm_home
       *
       * >
       */
      alarm_arm_home: (
        service_data?: {
          /**
           * ## code
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "code": "1234"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          code?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - alarm_control_panel
           * >     supported_features:
           * >       - 1
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### alarm_arm_night
       *
       * >
       */
      alarm_arm_night: (
        service_data?: {
          /**
           * ## code
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "code": "1234"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          code?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - alarm_control_panel
           * >     supported_features:
           * >       - 4
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### alarm_arm_vacation
       *
       * >
       */
      alarm_arm_vacation: (
        service_data?: {
          /**
           * ## code
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "code": "1234"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          code?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - alarm_control_panel
           * >     supported_features:
           * >       - 32
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### alarm_disarm
       *
       * >
       */
      alarm_disarm: (
        service_data?: {
          /**
           * ## code
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "code": "1234"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          code?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - alarm_control_panel
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### alarm_trigger
       *
       * >
       */
      alarm_trigger: (
        service_data?: {
          /**
           * ## code
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "code": "1234"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          code?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - alarm_control_panel
           * >     supported_features:
           * >       - 8
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: alexa_media
    alexa_media: {
      /**
       * ### Enable network discovery
       *
       * > Re-enable Alexa network discovery so the next polling cycle will rediscover Alexa devices for the selected accounts.
       * >
       */
      enable_network_discovery: (service_data?: {
        /**
                 * ## Account email(s)
                 *
                 * > Optional Alexa account email or list of emails. If omitted, all Alexa Media accounts will be refreshed.
                
                 *
                 * ### Example
                 *
                 * > ```json
                 * > {
                 * >   "email": "email protected"
                 * > }
                 * > ```
                 *
                 * ## Selector
                 *
                 * > ```yaml
                 * > text:
                 * >   multiple: false
                 * >   multiline: false
                 * > ```
                 */
        email?: string;
      }) => Promise<void>;
      /**
       * ### force_logout
       *
       * > Force logout of Alexa Login account and deletion of .pickle. Intended for debugging use.
       */
      force_logout: (service_data?: {
        /**
         * ## email
         *
         * > List of Alexa accounts to log out. If empty, will log out from all known accounts.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "email": "my_email@alexa.com"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * >
         * > ```
         */
        email?: unknown;
      }) => Promise<void>;
      /**
       * ### get_history_records
       *
       * > Returns the last entries of all the customer history.
       */
      get_history_records: (service_data?: {
        /**
         * ## Entity
         *
         * > Alexa Media Player device to get history from.
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - media_player
         * >   integration: alexa_media
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id:
          | PICK_FROM_PLATFORM<"alexa_media", "media_player">
          | PICK_FROM_PLATFORM<"alexa_media", "media_player">[];
        /**
         * ## Entries
         *
         * > Number of records to return.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entries": "5"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > 5
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * >
         * > ```
         */
        entries?: unknown;
      }) => Promise<void>;
      /**
       * ### restore_volume
       *
       * > Restores an Alexa Media Player volume level to the previous volume level.
       */
      restore_volume: (service_data: {
        /**
         * ## Entity
         *
         * > Alexa Media Player device to restore volume on.
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - media_player
         * >   integration: alexa_media
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id:
          | PICK_FROM_PLATFORM<"alexa_media", "media_player">
          | PICK_FROM_PLATFORM<"alexa_media", "media_player">[];
      }) => Promise<void>;
      /**
       * ### update_last_called
       *
       * > Forces update of last_called echo device for each Alexa account.
       */
      update_last_called: (service_data?: {
        /**
         * ## email
         *
         * > List of Alexa accounts to update. If empty, will update all known accounts.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "email": "my_email@alexa.com"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * >
         * > ```
         */
        email?: unknown;
      }) => Promise<void>;
    };
    // # MARK: apple_tv
    apple_tv: {
      /**
       * ### append_keyboard_text
       *
       * >
       */
      append_keyboard_text: (service_data: {
        /**
         * ## config_entry_id
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: apple_tv
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## text
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        text: string;
      }) => Promise<void>;
      /**
       * ### clear_keyboard_text
       *
       * >
       */
      clear_keyboard_text: (service_data: {
        /**
         * ## config_entry_id
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: apple_tv
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
      }) => Promise<void>;
      /**
       * ### set_keyboard_text
       *
       * >
       */
      set_keyboard_text: (service_data: {
        /**
         * ## config_entry_id
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: apple_tv
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## text
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        text: string;
      }) => Promise<void>;
    };
    // # MARK: assist_satellite
    assist_satellite: {
      /**
       * ### announce
       *
       * >
       */
      announce: (
        service_data?: {
          /**
           * ## media_id
           *
           * ## Selector
           *
           * > ```yaml
           * > media:
           * >   accept:
           * >     - audio/*
           * >   multiple: false
           * > ```
           */
          media_id?: string;
          /**
           * ## message
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "message": "Time to wake up!"
           * > }
           * > ```
           *
           * ### Default
           *
           * > ```json
           * > ""
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          message?: string;
          /**
           * ## preannounce
           *
           * ### Default
           *
           * > ```json
           * > true
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          preannounce?: boolean;
          /**
           * ## preannounce_media_id
           *
           * ## Selector
           *
           * > ```yaml
           * > media:
           * >   accept:
           * >     - audio/*
           * >   multiple: false
           * > ```
           */
          preannounce_media_id?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - assist_satellite
           * >     supported_features:
           * >       - 1
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### ask_question
       *
       * >
       */
      ask_question: <T = unknown>(service_data?: {
        /**
         * ## answers
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   label_field: sentences
         * >   description_field: id
         * >   multiple: true
         * >   translation_key: answers
         * >   fields:
         * >     id:
         * >       required: true
         * >       selector:
         * >         text:
         * >           multiple: false
         * >           multiline: false
         * >     sentences:
         * >       required: true
         * >       selector:
         * >         text:
         * >           multiple: true
         * >           multiline: false
         * > ```
         */
        answers?: Record<string, unknown> | unknown[];
        /**
         * ## entity_id
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   filter:
         * >     - domain:
         * >         - assist_satellite
         * >       supported_features:
         * >         - 2
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id: PICK_ENTITY | PICK_ENTITY[];
        /**
         * ## preannounce
         *
         * ### Default
         *
         * > ```json
         * > true
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        preannounce?: boolean;
        /**
         * ## preannounce_media_id
         *
         * ## Selector
         *
         * > ```yaml
         * > media:
         * >   accept:
         * >     - audio/*
         * >   multiple: false
         * > ```
         */
        preannounce_media_id?: string;
        /**
         * ## question
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "question": "What kind of music would you like to play?"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > ""
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        question?: string;
        /**
         * ## question_media_id
         *
         * ## Selector
         *
         * > ```yaml
         * > media:
         * >   accept:
         * >     - audio/*
         * >   multiple: false
         * > ```
         */
        question_media_id?: string;
      }) => Promise<T>;
      /**
       * ### start_conversation
       *
       * >
       */
      start_conversation: (
        service_data?: {
          /**
           * ## extra_system_prompt
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          extra_system_prompt?: string;
          /**
           * ## preannounce
           *
           * ### Default
           *
           * > ```json
           * > true
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          preannounce?: boolean;
          /**
           * ## preannounce_media_id
           *
           * ## Selector
           *
           * > ```yaml
           * > media:
           * >   accept:
           * >     - audio/*
           * >   multiple: false
           * > ```
           */
          preannounce_media_id?: string;
          /**
           * ## start_media_id
           *
           * ## Selector
           *
           * > ```yaml
           * > media:
           * >   accept:
           * >     - audio/*
           * >   multiple: false
           * > ```
           */
          start_media_id?: string;
          /**
           * ## start_message
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "start_message": "You left the lights on in the living room. Turn them off?"
           * > }
           * > ```
           *
           * ### Default
           *
           * > ```json
           * > ""
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          start_message?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - assist_satellite
           * >     supported_features:
           * >       - 2
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: automation
    automation: {
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### toggle
       *
       * >
       */
      toggle: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - automation
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### trigger
       *
       * >
       */
      trigger: (
        service_data?: {
          /**
           * ## skip_condition
           *
           * ### Default
           *
           * > ```json
           * > true
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          skip_condition?: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - automation
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_off
       *
       * >
       */
      turn_off: (
        service_data?: {
          /**
           * ## stop_actions
           *
           * ### Default
           *
           * > ```json
           * > true
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          stop_actions?: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - automation
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - automation
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: backup
    backup: {
      /**
       * ### create_automatic
       *
       * >
       */
      create_automatic: (service_data: EmptyObject) => Promise<void>;
    };
    // # MARK: button
    button: {
      /**
       * ### press
       *
       * >
       */
      press: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - button
           * > ```
           */
          entity_id: PICK_ENTITY<"button"> | PICK_ENTITY<"button">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: calendar
    calendar: {
      /**
       * ### create_event
       *
       * >
       */
      create_event: (
        service_data?: {
          /**
           * ## description
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "description": "Meeting to provide technical review for 'Phoenix' design."
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          description?: string;
          /**
           * ## end_date
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "end_date": "2022-03-23"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > date: {}
           * > ```
           */
          end_date?: string;
          /**
           * ## end_date_time
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "end_date_time": "2022-03-22 22:00:00"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > datetime: {}
           * > ```
           */
          end_date_time?: string;
          /**
           * ## in
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "in": "{\"days\": 2} or {\"weeks\": 2}"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * >
           * > ```
           */
          in?: unknown;
          /**
           * ## location
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "location": "Conference Room - F123, Bldg. 002"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          location?: string;
          /**
           * ## start_date
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "start_date": "2022-03-22"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > date: {}
           * > ```
           */
          start_date?: string;
          /**
           * ## start_date_time
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "start_date_time": "2022-03-22 20:00:00"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > datetime: {}
           * > ```
           */
          start_date_time?: string;
          /**
           * ## summary
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "summary": "Department Party"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          summary: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - calendar
           * >     supported_features:
           * >       - 1
           * > ```
           */
          entity_id: PICK_ENTITY<"calendar"> | PICK_ENTITY<"calendar">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### get_events
       *
       * >
       */
      get_events: <T = unknown>(
        service_data?: {
          /**
           * ## duration
           *
           * ## Selector
           *
           * > ```yaml
           * > duration:
           * >   enable_second: true
           * > ```
           */
          duration?: string;
          /**
           * ## end_date_time
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "end_date_time": "2022-03-22 22:00:00"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > datetime: {}
           * > ```
           */
          end_date_time?: string;
          /**
           * ## start_date_time
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "start_date_time": "2022-03-22 20:00:00"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > datetime: {}
           * > ```
           */
          start_date_time?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - calendar
           * > ```
           */
          entity_id: PICK_ENTITY<"calendar"> | PICK_ENTITY<"calendar">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<T>;
    };
    // # MARK: camera
    camera: {
      /**
       * ### disable_motion_detection
       *
       * >
       */
      disable_motion_detection: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - camera
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### enable_motion_detection
       *
       * >
       */
      enable_motion_detection: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - camera
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### play_stream
       *
       * >
       */
      play_stream: (
        service_data?: {
          /**
           * ## format
           *
           * ### Default
           *
           * > ```json
           * > "hls"
           * > ```
           */
          format?: "hls";
          /**
           * ## media_player
           *
           * ## Selector
           *
           * > ```yaml
           * > entity:
           * >   domain:
           * >     - media_player
           * >   reorder: false
           * >   multiple: false
           * > ```
           */
          media_player: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - camera
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### record
       *
       * >
       */
      record: (
        service_data?: {
          /**
           * ## duration
           *
           * ### Default
           *
           * > ```json
           * > 30
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 1
           * >   max: 3600
           * >   unit_of_measurement: seconds
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          duration?: number;
          /**
           * ## filename
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "filename": "/tmp/snapshot_{{ entity_id.name }}.mp4"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          filename: string;
          /**
           * ## lookback
           *
           * ### Default
           *
           * > ```json
           * > 0
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 300
           * >   unit_of_measurement: seconds
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          lookback?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - camera
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### snapshot
       *
       * >
       */
      snapshot: (
        service_data: {
          /**
           * ## filename
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "filename": "/tmp/snapshot_{{ entity_id.name }}.jpg"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          filename: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - camera
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_off
       *
       * >
       */
      turn_off: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - camera
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - camera
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: chime_tts
    chime_tts: {
      /**
       * ### Clear Cache
       *
       * > Remove text-to-speech cache files from Chime TTS and/or Home Assistant.
       */
      clear_cache: (service_data?: {
        /**
         * ## Temporary Chimes Cache
         *
         * > Remove the cached local chime files downloaded by Chime TTS
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "clear_chimes_cache": "True"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > true
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        clear_chimes_cache?: boolean;
        /**
         * ## Home Assistant TTS Cache
         *
         * > Remove the TTS audio files stored in the Home Assistant TTS cache
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "clear_ha_tts_cache": "True"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > true
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        clear_ha_tts_cache?: boolean;
        /**
         * ## Temporary Chime TTS Cache
         *
         * > Remove the local temporary audio files stored in the Chime TTS cache
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "clear_temp_tts_cache": "True"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > true
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        clear_temp_tts_cache?: boolean;
        /**
         * ## Publicly Accessible Chime TTS Cache
         *
         * > Remove the publicly accessible audio files stored in the Chime TTS cache
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "clear_www_tts_cache": "True"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > true
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        clear_www_tts_cache?: boolean;
      }) => Promise<void>;
      /**
       * ### Replay
       *
       * > Replay the last service call to chime_tts.say with the same parameters
       */
      replay: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### Say
       *
       * > Play an audio file before TTS audio
       */
      say: (
        service_data?: {
          /**
           * ## Announce
           *
           * > Reduce volume of currently playing audio during during announcement (on supported devices)
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "announce": "True"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          announce?: boolean;
          /**
           * ## Audio Conversion
           *
           * > Convert the audio to match Alexa speaker requirements, or use your own FFmpeg arguments
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "audio_conversion": "Alexa"
           * > }
           * > ```
           */
          audio_conversion?: LiteralUnion<"Alexa" | "Volume 100%" | "Custom", string>;
          /**
           * ## Cache
           *
           * > Whether or not to save/reuse the generated audio file in a local cache
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "cache": "True"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          cache?: boolean;
          /**
           * ## Chime Path
           *
           * > A preset or custom audio file to be played before TTS audio
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "chime_path": "custom_components/chime_tts/mp3s/bells.mp3"
           * > }
           * > ```
           */
          chime_path?: LiteralUnion<
            | "ba_dum_tss"
            | "bells"
            | "bells_2"
            | "bright"
            | "chirp"
            | "choir"
            | "chord"
            | "classical"
            | "crickets"
            | "ding_dong"
            | "drumroll"
            | "dun_dun_dun"
            | "error"
            | "fanfare"
            | "glockenspiel"
            | "hail"
            | "knock"
            | "marimba"
            | "mario_coin"
            | "microphone_tap"
            | "sad_trombone"
            | "soft"
            | "tada"
            | "toast"
            | "twenty_four"
            | "whistle"
            | "",
            string
          >;
          /**
           * ## Crossfade
           *
           * > Crossfade (in milliseconds) between audio segments
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "crossfade": "1000"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   mode: box
           * >   step: 1
           * >   unit_of_measurement: ms
           * > ```
           */
          crossfade?: number;
          /**
           * ## End Chime Path
           *
           * > A preset or custom audio file to be played after TTS audio
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "end_chime_path": "custom_components/chime_tts/mp3s/tada.mp3"
           * > }
           * > ```
           */
          end_chime_path?: LiteralUnion<
            | "ba_dum_tss"
            | "bells"
            | "bells_2"
            | "bright"
            | "chirp"
            | "choir"
            | "chord"
            | "classical"
            | "crickets"
            | "ding_dong"
            | "drumroll"
            | "dun_dun_dun"
            | "error"
            | "fanfare"
            | "glockenspiel"
            | "hail"
            | "knock"
            | "marimba"
            | "mario_coin"
            | "microphone_tap"
            | "sad_trombone"
            | "soft"
            | "tada"
            | "toast"
            | "twenty_four"
            | "whistle"
            | "",
            string
          >;
          /**
           * ## Fade Audio
           *
           * > Fade out playing audio during announcement, fade back in when completed (on supported devices)
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "fade_audio": "True"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          fade_audio?: boolean;
          /**
           * ## Final Delay
           *
           * > Final delay (in milliseconds) added to the end of the audio
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "final_delay": "100"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   max: 10000
           * >   min: 0
           * >   mode: box
           * >   step: 1
           * >   unit_of_measurement: ms
           * > ```
           */
          final_delay?: number;
          /**
           * ## Join Players
           *
           * > Join media_players for simultaneous playback (for supported speakers)
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "join_players": "True"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          join_players?: boolean;
          /**
           * ## Language
           *
           * > The TTS language (supported by Google Translate, Microsoft Edge TTS, Amazon Polly and Nabu Casa Cloud TTS)
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "language": "en"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          language?: string;
          /**
           * ## Message
           *
           * > Text converted into TTS audio
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "message": "Hello world!"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > template: {}
           * > ```
           */
          message?: string;
          /**
           * ## Offset
           *
           * > Adds a delay between audio segments when value > 0, or overlays audio segments when value < 0.
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "offset": "450"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   max: 10000
           * >   min: -10000
           * >   mode: box
           * >   step: 10
           * >   unit_of_measurement: ms
           * > ```
           */
          offset?: number;
          /**
           * ## Options
           *
           * > YAML Options to pass to TTS services (will override `tld` and `voice` fields)
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "options": "tld: com.au\u000boice: en-AU"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiline: true
           * >   multiple: false
           * > ```
           */
          options?: string;
          /**
           * ## TLD
           *
           * > The dialect (supported by Google Translate)
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "tld": "com.au"
           * > }
           * > ```
           */
          tld?:
            | "com"
            | "co.uk"
            | "com.au"
            | "ca"
            | "co.in"
            | "ie"
            | "co.za"
            | "fr"
            | "com.br"
            | "pt"
            | "es";
          /**
           * ## TTS Pitch
           *
           * > Change the the TTS pitch in semitones. Negative values for lower, positive for higher
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "tts_pitch": "3"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   max: 100
           * >   min: -100
           * >   mode: slider
           * >   step: 1
           * >   unit_of_measurement: semitones
           * > ```
           */
          tts_pitch?: number;
          /**
           * ## TTS Platform
           *
           * > TTS platform used to generate TTS audio
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "tts_platform": "google_translate"
           * > }
           * > ```
           */
          tts_platform?: LiteralUnion<
            | "amazon_polly"
            | "baidu"
            | "tts.elevenlabs"
            | "google_cloud"
            | "google_translate"
            | "watson_tts"
            | "marytts"
            | "edge_tts"
            | "microsoft"
            | "cloud"
            | "openai_tts"
            | "picotts"
            | "tts.piper"
            | "voicerss"
            | "yandextts",
            string
          >;
          /**
           * ## TTS Speed
           *
           * > Set the speed of the TTS audio to between 1% and 500% of the original
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "tts_speed": "125"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   max: 500
           * >   min: 1
           * >   mode: slider
           * >   step: 5
           * >   unit_of_measurement: '%'
           * > ```
           */
          tts_speed?: number;
          /**
           * ## Unjoin Players
           *
           * > Release the joined media_players after playback
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "unjoin_players": "True"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          unjoin_players?: boolean;
          /**
           * ## Voice
           *
           * > Define the voice for the TTS audio (on supported TTS platforms)
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "voice": "en-AU"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          voice?: string;
          /**
           * ## Volume Level
           *
           * > The volume to use when playing audio
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "volume_level": "0.75"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   max: 1
           * >   min: 0
           * >   mode: slider
           * >   step: 0.01
           * > ```
           */
          volume_level?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 512
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### Say URL
       *
       * > Generates an audio file with the `chime_tts.say` service and returns either an external URL or a local file path, depending on the folder set in the configuration
       */
      say_url: <T = unknown>(service_data?: {
        /**
         * ## Audio Conversion
         *
         * > Convert the audio to match Alexa speaker requirements, or use your own FFmpeg arguments
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "audio_conversion": "Alexa"
         * > }
         * > ```
         */
        audio_conversion?: LiteralUnion<"Alexa" | "Volume 100%" | "Custom", string>;
        /**
         * ## Cache
         *
         * > Whether or not to save/reuse the generated audio file in a local cache
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "cache": "True"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        cache?: boolean;
        /**
         * ## Chime Path
         *
         * > A preset or custom audio file to be played before TTS audio
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "chime_path": "custom_components/chime_tts/mp3s/bells.mp3"
         * > }
         * > ```
         */
        chime_path?: LiteralUnion<
          | "ba_dum_tss"
          | "bells"
          | "bells_2"
          | "bright"
          | "chirp"
          | "choir"
          | "chord"
          | "classical"
          | "crickets"
          | "ding_dong"
          | "drumroll"
          | "dun_dun_dun"
          | "error"
          | "fanfare"
          | "glockenspiel"
          | "hail"
          | "knock"
          | "marimba"
          | "mario_coin"
          | "microphone_tap"
          | "sad_trombone"
          | "soft"
          | "tada"
          | "toast"
          | "twenty_four"
          | "whistle"
          | "",
          string
        >;
        /**
         * ## Crossfade
         *
         * > Crossfade (in milliseconds) between audio segments
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "crossfade": "1000"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 0
         * >   mode: box
         * >   step: 1
         * >   unit_of_measurement: ms
         * > ```
         */
        crossfade?: number;
        /**
         * ## End Chime Path
         *
         * > A preset or custom audio file to be played after TTS audio
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "end_chime_path": "custom_components/chime_tts/mp3s/tada.mp3"
         * > }
         * > ```
         */
        end_chime_path?: LiteralUnion<
          | "ba_dum_tss"
          | "bells"
          | "bells_2"
          | "bright"
          | "chirp"
          | "choir"
          | "chord"
          | "classical"
          | "crickets"
          | "ding_dong"
          | "drumroll"
          | "dun_dun_dun"
          | "error"
          | "fanfare"
          | "glockenspiel"
          | "hail"
          | "knock"
          | "marimba"
          | "mario_coin"
          | "microphone_tap"
          | "sad_trombone"
          | "soft"
          | "tada"
          | "toast"
          | "twenty_four"
          | "whistle"
          | "",
          string
        >;
        /**
         * ## Final Delay
         *
         * > Final delay (in milliseconds) added to the end of the audio
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "final_delay": "100"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   max: 10000
         * >   min: 0
         * >   mode: box
         * >   step: 1
         * >   unit_of_measurement: ms
         * > ```
         */
        final_delay?: number;
        /**
         * ## Language
         *
         * > The TTS language (supported by Google Translate, Microsoft Edge TTS and Nabu Casa Cloud TTS)
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "language": "en"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        language?: string;
        /**
         * ## Message
         *
         * > Text converted into TTS audio
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "Hello world!"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > template: {}
         * > ```
         */
        message?: string;
        /**
         * ## Offset
         *
         * > Adds a delay between audio segments when value > 0, or overlays audio segments when value < 0.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "offset": "450"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   max: 10000
         * >   min: -10000
         * >   mode: box
         * >   step: 10
         * >   unit_of_measurement: ms
         * > ```
         */
        offset?: number;
        /**
         * ## Options
         *
         * > YAML Options to pass to TTS services (will override `tld` and `voice` fields)
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "options": "tld: com.au\u000boice: en-AU"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiline: true
         * >   multiple: false
         * > ```
         */
        options?: string;
        /**
         * ## TLD
         *
         * > The dialect (supported by Google Translate)
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "tld": "com.au"
         * > }
         * > ```
         */
        tld?:
          | "com"
          | "co.uk"
          | "com.au"
          | "ca"
          | "co.in"
          | "ie"
          | "co.za"
          | "fr"
          | "com.br"
          | "pt"
          | "es";
        /**
         * ## TTS Pitch
         *
         * > Change the the TTS pitch in semitones. Negative values for lower, positive for higher
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "tts_pitch": "3"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   max: 100
         * >   min: -100
         * >   mode: slider
         * >   step: 1
         * >   unit_of_measurement: semitones
         * > ```
         */
        tts_pitch?: number;
        /**
         * ## TTS Platform
         *
         * > TTS platform used to generate TTS audio
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "tts_platform": "google_translate"
         * > }
         * > ```
         */
        tts_platform?: LiteralUnion<
          | "amazon_polly"
          | "baidu"
          | "tts.elevenlabs"
          | "google_cloud"
          | "google_translate"
          | "watson_tts"
          | "marytts"
          | "edge_tts"
          | "microsoft"
          | "cloud"
          | "openai_tts"
          | "picotts"
          | "tts.piper"
          | "voicerss"
          | "yandextts",
          string
        >;
        /**
         * ## TTS Speed
         *
         * > Set the speed of the TTS audio to between 1% and 500% of the original
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "tts_speed": "125"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   max: 500
         * >   min: 1
         * >   mode: slider
         * >   step: 5
         * >   unit_of_measurement: '%'
         * > ```
         */
        tts_speed?: number;
        /**
         * ## Voice
         *
         * > Define the voice for the TTS audio (on supported TTS platforms)
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "voice": "en-AU"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        voice?: string;
      }) => Promise<T>;
    };
    // # MARK: climate
    climate: {
      /**
       * ### set_fan_mode
       *
       * >
       */
      set_fan_mode: (
        service_data: {
          /**
           * ## fan_mode
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "fan_mode": "low"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   attribute: fan_mode
           * >   multiple: false
           * > ```
           */
          fan_mode: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - climate
           * >     supported_features:
           * >       - 8
           * > ```
           */
          entity_id: PICK_ENTITY<"climate"> | PICK_ENTITY<"climate">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_humidity
       *
       * >
       */
      set_humidity: (
        service_data: {
          /**
           * ## humidity
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 30
           * >   max: 99
           * >   unit_of_measurement: '%'
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          humidity: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - climate
           * >     supported_features:
           * >       - 4
           * > ```
           */
          entity_id: PICK_ENTITY<"climate"> | PICK_ENTITY<"climate">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_hvac_mode
       *
       * >
       */
      set_hvac_mode: (
        service_data?: {
          /**
           * ## hvac_mode
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   hide_states:
           * >     - unavailable
           * >     - unknown
           * >   multiple: false
           * > ```
           */
          hvac_mode?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - climate
           * > ```
           */
          entity_id: PICK_ENTITY<"climate"> | PICK_ENTITY<"climate">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_preset_mode
       *
       * >
       */
      set_preset_mode: (
        service_data: {
          /**
           * ## preset_mode
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "preset_mode": "away"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   attribute: preset_mode
           * >   multiple: false
           * > ```
           */
          preset_mode: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - climate
           * >     supported_features:
           * >       - 16
           * > ```
           */
          entity_id: PICK_ENTITY<"climate"> | PICK_ENTITY<"climate">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_swing_horizontal_mode
       *
       * >
       */
      set_swing_horizontal_mode: (
        service_data: {
          /**
           * ## swing_horizontal_mode
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "swing_horizontal_mode": "on"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   attribute: swing_horizontal_mode
           * >   multiple: false
           * > ```
           */
          swing_horizontal_mode: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - climate
           * >     supported_features:
           * >       - 512
           * > ```
           */
          entity_id: PICK_ENTITY<"climate"> | PICK_ENTITY<"climate">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_swing_mode
       *
       * >
       */
      set_swing_mode: (
        service_data: {
          /**
           * ## swing_mode
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "swing_mode": "on"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   attribute: swing_mode
           * >   multiple: false
           * > ```
           */
          swing_mode: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - climate
           * >     supported_features:
           * >       - 32
           * > ```
           */
          entity_id: PICK_ENTITY<"climate"> | PICK_ENTITY<"climate">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_temperature
       *
       * >
       */
      set_temperature: (
        service_data?: {
          /**
           * ## hvac_mode
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   hide_states:
           * >     - unavailable
           * >     - unknown
           * >   multiple: false
           * > ```
           */
          hvac_mode?: string;
          /**
           * ## target_temp_high
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 250
           * >   step: 0.1
           * >   mode: box
           * > ```
           */
          target_temp_high?: number;
          /**
           * ## target_temp_low
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 250
           * >   step: 0.1
           * >   mode: box
           * > ```
           */
          target_temp_low?: number;
          /**
           * ## temperature
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 250
           * >   step: 0.1
           * >   mode: box
           * > ```
           */
          temperature?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - climate
           * >     supported_features:
           * >       - 1
           * >       - 2
           * > ```
           */
          entity_id: PICK_ENTITY<"climate"> | PICK_ENTITY<"climate">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### toggle
       *
       * >
       */
      toggle: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - climate
           * >     supported_features:
           * >       - 128
           * >       - 256
           * > ```
           */
          entity_id: PICK_ENTITY<"climate"> | PICK_ENTITY<"climate">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_off
       *
       * >
       */
      turn_off: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - climate
           * >     supported_features:
           * >       - 128
           * > ```
           */
          entity_id: PICK_ENTITY<"climate"> | PICK_ENTITY<"climate">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - climate
           * >     supported_features:
           * >       - 256
           * > ```
           */
          entity_id: PICK_ENTITY<"climate"> | PICK_ENTITY<"climate">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: cloud
    cloud: {
      /**
       * ### remote_connect
       *
       * >
       */
      remote_connect: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### remote_disconnect
       *
       * >
       */
      remote_disconnect: (service_data: EmptyObject) => Promise<void>;
    };
    // # MARK: conversation
    conversation: {
      /**
       * ### process
       *
       * >
       */
      process: <T = unknown>(service_data?: {
        /**
         * ## agent_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "agent_id": "homeassistant"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > conversation_agent: {}
         * > ```
         */
        agent_id?: "conversation.home_assistant" | "conversation.openai_conversation";
        /**
         * ## conversation_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "conversation_id": "my_conversation_1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        conversation_id?: string;
        /**
         * ## language
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "language": "NL"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        language?: string;
        /**
         * ## text
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "text": "Turn all lights on"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        text: string;
      }) => Promise<T>;
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data?: {
        /**
         * ## agent_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "agent_id": "homeassistant"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > conversation_agent: {}
         * > ```
         */
        agent_id?: "conversation.home_assistant" | "conversation.openai_conversation";
        /**
         * ## language
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "language": "NL"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        language?: string;
      }) => Promise<void>;
    };
    // # MARK: counter
    counter: {
      /**
       * ### decrement
       *
       * >
       */
      decrement: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - counter
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### increment
       *
       * >
       */
      increment: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - counter
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### reset
       *
       * >
       */
      reset: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - counter
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_value
       *
       * >
       */
      set_value: (
        service_data: {
          /**
           * ## value
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 9223372036854776000
           * >   mode: box
           * >   step: 1
           * > ```
           */
          value: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - counter
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: cover
    cover: {
      /**
       * ### close_cover
       *
       * >
       */
      close_cover: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - cover
           * >     supported_features:
           * >       - 2
           * > ```
           */
          entity_id: PICK_ENTITY<"cover"> | PICK_ENTITY<"cover">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### close_cover_tilt
       *
       * >
       */
      close_cover_tilt: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - cover
           * >     supported_features:
           * >       - 32
           * > ```
           */
          entity_id: PICK_ENTITY<"cover"> | PICK_ENTITY<"cover">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### open_cover
       *
       * >
       */
      open_cover: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - cover
           * >     supported_features:
           * >       - 1
           * > ```
           */
          entity_id: PICK_ENTITY<"cover"> | PICK_ENTITY<"cover">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### open_cover_tilt
       *
       * >
       */
      open_cover_tilt: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - cover
           * >     supported_features:
           * >       - 16
           * > ```
           */
          entity_id: PICK_ENTITY<"cover"> | PICK_ENTITY<"cover">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_cover_position
       *
       * >
       */
      set_cover_position: (
        service_data: {
          /**
           * ## position
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 100
           * >   unit_of_measurement: '%'
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          position: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - cover
           * >     supported_features:
           * >       - 4
           * > ```
           */
          entity_id: PICK_ENTITY<"cover"> | PICK_ENTITY<"cover">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_cover_tilt_position
       *
       * >
       */
      set_cover_tilt_position: (
        service_data: {
          /**
           * ## tilt_position
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 100
           * >   unit_of_measurement: '%'
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          tilt_position: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - cover
           * >     supported_features:
           * >       - 128
           * > ```
           */
          entity_id: PICK_ENTITY<"cover"> | PICK_ENTITY<"cover">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### stop_cover
       *
       * >
       */
      stop_cover: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - cover
           * >     supported_features:
           * >       - 8
           * > ```
           */
          entity_id: PICK_ENTITY<"cover"> | PICK_ENTITY<"cover">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### stop_cover_tilt
       *
       * >
       */
      stop_cover_tilt: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - cover
           * >     supported_features:
           * >       - 64
           * > ```
           */
          entity_id: PICK_ENTITY<"cover"> | PICK_ENTITY<"cover">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### toggle
       *
       * >
       */
      toggle: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - cover
           * >     supported_features:
           * >       - 3
           * > ```
           */
          entity_id: PICK_ENTITY<"cover"> | PICK_ENTITY<"cover">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### toggle_cover_tilt
       *
       * >
       */
      toggle_cover_tilt: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - cover
           * >     supported_features:
           * >       - 48
           * > ```
           */
          entity_id: PICK_ENTITY<"cover"> | PICK_ENTITY<"cover">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: date
    date: {
      /**
       * ### set_value
       *
       * >
       */
      set_value: (
        service_data: {
          /**
           * ## date
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "date": "2022/11/01"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > date: {}
           * > ```
           */
          date: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - date
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: datetime
    datetime: {
      /**
       * ### set_value
       *
       * >
       */
      set_value: (
        service_data: {
          /**
           * ## datetime
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "datetime": "2023-10-07T21:35:22"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > datetime: {}
           * > ```
           */
          datetime: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - datetime
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: device_tracker
    device_tracker: {
      /**
       * ### see
       *
       * >
       */
      see: (service_data?: {
        /**
         * ## battery
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 0
         * >   max: 100
         * >   unit_of_measurement: '%'
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        battery?: number;
        /**
         * ## dev_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "dev_id": "phonedave"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        dev_id?: string;
        /**
         * ## gps
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "gps": "[51.509802, -0.086692]"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        gps?: Record<string, unknown> | unknown[];
        /**
         * ## gps_accuracy
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 0
         * >   mode: box
         * >   unit_of_measurement: m
         * >   step: 1
         * > ```
         */
        gps_accuracy?: number;
        /**
         * ## host_name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "host_name": "Dave"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        host_name?: string;
        /**
         * ## location_name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "location_name": "home"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        location_name?: string;
        /**
         * ## mac
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "mac": "FF:FF:FF:FF:FF:FF"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        mac?: string;
      }) => Promise<void>;
    };
    // # MARK: fan
    fan: {
      /**
       * ### decrease_speed
       *
       * >
       */
      decrease_speed: (
        service_data?: {
          /**
           * ## percentage_step
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 100
           * >   unit_of_measurement: '%'
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          percentage_step?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - fan
           * >     supported_features:
           * >       - 1
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### increase_speed
       *
       * >
       */
      increase_speed: (
        service_data?: {
          /**
           * ## percentage_step
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 100
           * >   unit_of_measurement: '%'
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          percentage_step?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - fan
           * >     supported_features:
           * >       - 1
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### oscillate
       *
       * >
       */
      oscillate: (
        service_data: {
          /**
           * ## oscillating
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          oscillating: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - fan
           * >     supported_features:
           * >       - 2
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_direction
       *
       * >
       */
      set_direction: (
        service_data: {
          /**
           * ## direction
           */
          direction: "forward" | "reverse";
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - fan
           * >     supported_features:
           * >       - 4
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_percentage
       *
       * >
       */
      set_percentage: (
        service_data: {
          /**
           * ## percentage
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 100
           * >   unit_of_measurement: '%'
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          percentage: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - fan
           * >     supported_features:
           * >       - 1
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_preset_mode
       *
       * >
       */
      set_preset_mode: (
        service_data: {
          /**
           * ## preset_mode
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "preset_mode": "auto"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   attribute: preset_mode
           * >   multiple: false
           * > ```
           */
          preset_mode: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - fan
           * >     supported_features:
           * >       - 8
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### toggle
       *
       * >
       */
      toggle: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - fan
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_off
       *
       * >
       */
      turn_off: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - fan
           * >     supported_features:
           * >       - 16
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (
        service_data?: {
          /**
           * ## percentage
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 100
           * >   unit_of_measurement: '%'
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          percentage?: number;
          /**
           * ## preset_mode
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "preset_mode": "auto"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   attribute: preset_mode
           * >   multiple: false
           * > ```
           */
          preset_mode?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - fan
           * >     supported_features:
           * >       - 32
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: ffmpeg
    ffmpeg: {
      /**
       * ### restart
       *
       * >
       */
      restart: (service_data?: {
        /**
         * ## entity_id
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   integration: ffmpeg
         * >   domain:
         * >     - binary_sensor
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id?: never | never[];
      }) => Promise<void>;
      /**
       * ### start
       *
       * >
       */
      start: (service_data?: {
        /**
         * ## entity_id
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   integration: ffmpeg
         * >   domain:
         * >     - binary_sensor
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id?: never | never[];
      }) => Promise<void>;
      /**
       * ### stop
       *
       * >
       */
      stop: (service_data?: {
        /**
         * ## entity_id
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   integration: ffmpeg
         * >   domain:
         * >     - binary_sensor
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id?: never | never[];
      }) => Promise<void>;
    };
    // # MARK: file
    file: {
      /**
       * ### read_file
       *
       * >
       */
      read_file: <T = unknown>(service_data?: {
        /**
         * ## file_encoding
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "file_encoding": "JSON"
         * > }
         * > ```
         */
        file_encoding?: "JSON" | "YAML";
        /**
         * ## file_name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "file_name": "www/my_file.json"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        file_name?: string;
      }) => Promise<T>;
    };
    // # MARK: frontend
    frontend: {
      /**
       * ### reload_themes
       *
       * >
       */
      reload_themes: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### set_theme
       *
       * >
       */
      set_theme: (service_data?: {
        /**
         * ## name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "name": "default"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > theme:
         * >   include_default: true
         * > ```
         */
        name?: string;
        /**
         * ## name_dark
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "name_dark": "default"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > theme:
         * >   include_default: true
         * > ```
         */
        name_dark?: string;
      }) => Promise<void>;
    };
    // # MARK: generic_thermostat
    generic_thermostat: {
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
    };
    // # MARK: google
    google: {
      /**
       * ### create_event
       *
       * >
       */
      create_event: (
        service_data?: {
          /**
           * ## description
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "description": "Birthday bowling"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          description?: string;
          /**
           * ## end_date
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "end_date": "2022-03-11"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          end_date?: string;
          /**
           * ## end_date_time
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "end_date_time": "2022-03-22 22:00:00"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          end_date_time?: string;
          /**
           * ## in
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "in": "\"days\": 2 or \"weeks\": 2"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > object:
           * >   multiple: false
           * > ```
           */
          in?: Record<string, unknown> | unknown[];
          /**
           * ## location
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "location": "Conference Room - F123, Bldg. 002"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          location?: string;
          /**
           * ## start_date
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "start_date": "2022-03-10"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          start_date?: string;
          /**
           * ## start_date_time
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "start_date_time": "2022-03-22 20:00:00"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          start_date_time?: string;
          /**
           * ## summary
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "summary": "Bowling"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          summary: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - integration: google
           * >     domain:
           * >       - calendar
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"google", "calendar">
            | PICK_FROM_PLATFORM<"google", "calendar">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: google_mail
    google_mail: {
      /**
       * ### set_vacation
       *
       * >
       */
      set_vacation: (
        service_data?: {
          /**
           * ## enabled
           *
           * ### Default
           *
           * > ```json
           * > true
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          enabled: boolean;
          /**
           * ## end
           *
           * ## Selector
           *
           * > ```yaml
           * > date: {}
           * > ```
           */
          end?: string;
          /**
           * ## message
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          message: string;
          /**
           * ## plain_text
           *
           * ### Default
           *
           * > ```json
           * > true
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          plain_text?: boolean;
          /**
           * ## restrict_contacts
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          restrict_contacts?: boolean;
          /**
           * ## restrict_domain
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          restrict_domain?: boolean;
          /**
           * ## start
           *
           * ## Selector
           *
           * > ```yaml
           * > date: {}
           * > ```
           */
          start?: string;
          /**
           * ## title
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          title?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - integration: google_mail
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: group
    group: {
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### remove
       *
       * >
       */
      remove: (service_data: {
        /**
         * ## object_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "object_id": "test_group"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        object_id: Record<string, unknown> | unknown[];
      }) => Promise<void>;
      /**
       * ### set
       *
       * >
       */
      set: (service_data?: {
        /**
         * ## add_entities
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "add_entities": "domain.entity_id1, domain.entity_id2"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        add_entities?: PICK_ENTITY | PICK_ENTITY[];
        /**
         * ## all
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        all?: boolean;
        /**
         * ## entities
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entities": "domain.entity_id1, domain.entity_id2"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        entities?: PICK_ENTITY | PICK_ENTITY[];
        /**
         * ## icon
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "icon": "mdi:camera"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > icon: {}
         * > ```
         */
        icon?: string;
        /**
         * ## name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "name": "My test group"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        name?: string;
        /**
         * ## object_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "object_id": "test_group"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        object_id: string;
        /**
         * ## remove_entities
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "remove_entities": "domain.entity_id1, domain.entity_id2"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        remove_entities?: PICK_ENTITY | PICK_ENTITY[];
      }) => Promise<void>;
    };
    // # MARK: ha_optimizer
    ha_optimizer: {
      /**
       * ### Analyze Add-ons
       *
       * > Queries the Supervisor API to return host resource usage (CPU, RAM, disk, OS) and the full add-on list with per-addon state, version, update availability, and runtime stats (CPU %, RAM). Only available on Home Assistant OS / Supervised.
       * >
       */
      analyze_addons: <T = unknown>(service_data: EmptyObject) => Promise<T>;
      /**
       * ### Analyze Dashboard
       *
       * > Reads Lovelace dashboard config from .storage/lovelace* to detect heavy cards, overloaded views, missing or unavailable entities, duplicate entity references, uninstalled custom cards, Jinja2 template cards, WebSocket push pressure, and recorder cross-ref waste. Supports all default and named dashboards.
       * >
       */
      analyze_dashboard: <T = unknown>(service_data: EmptyObject) => Promise<T>;
      /**
       * ### Analyze Automation Dead Code
       *
       * > Scans all UI-created automations for broken triggers, actions, and conditions that reference entities, devices, or services that no longer exist. Helps identify automations that will silently fail.
       * >
       */
      analyze_dead_code: <T = unknown>(service_data: EmptyObject) => Promise<T>;
      /**
       * ### Analyze Fingerprint
       *
       * > Compares today's HA system behaviour against its own historical baseline (up to 30 days). Detects anomalies in state writes, automation trigger count, unavailability events, and HA lifecycle events using statistical methods (σ or IQR). Does not compare against other users — only against your own past behaviour.
       * >
       */
      analyze_fingerprint: <T = unknown>(service_data: EmptyObject) => Promise<T>;
      /**
       * ### Check Integration Health
       *
       * > Analyzes integration health by examining unavailability and reconnect patterns over the last 7 days. Identifies devices with weak batteries, poor signal, or abnormal disconnection rates and scores each integration.
       * >
       */
      analyze_health: <T = unknown>(service_data: EmptyObject) => Promise<T>;
      /**
       * ### Analyze Recorder DB
       *
       * > Queries the recorder database to identify top-writing entities, wasteful records (many writes, few distinct states), domain statistics, and generates a YAML snippet for recorder optimizations.
       * >
       */
      analyze_recorder: <T = unknown>(service_data: EmptyObject) => Promise<T>;
      /**
       * ### Detect State Storms
       *
       * > Finds entities that are updating their state abnormally fast compared to their domain baseline. High-frequency writers cause database bloat, increased CPU usage, and browser performance issues. Results include severity, ratio vs baseline, and fix suggestions.
       * >
       */
      analyze_storms: <T = unknown>(service_data: EmptyObject) => Promise<T>;
      /**
       * ### Collect Baseline
       *
       * > Captures a snapshot of yesterday's system metrics and saves it to the fingerprint baseline store. Normally runs automatically at 00:05 each day. Call manually on first install or when you want to collect data immediately without waiting for the scheduled run.
       * >
       */
      collect_baseline: <T = unknown>(service_data: EmptyObject) => Promise<T>;
      /**
       * ### Get last scan results
       *
       * > Returns the last scan results as a service response, including the current trash (soft-deleted entities). Use in automations or scripts to process results programmatically.
       * >
       */
      get_results: <T = unknown>(service_data: EmptyObject) => Promise<T>;
      /**
       * ### Purge selected entities
       *
       * > Purges (disables or permanently deletes) the specified entities. With soft_delete=true (default), entities are disabled and moved to the trash — fully reversible via the restore service. With soft_delete=false, entities are permanently deleted.
       * >
       */
      purge: (service_data?: {
        /**
         * ## Entity IDs
         *
         * > List of entity_ids to purge.
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        entity_ids: PICK_ENTITY | PICK_ENTITY[];
        /**
                 * ## Soft delete
                 *
                 * > If true (default), entities are disabled and tracked in trash (can be restored with the restore service). If false, entities are permanently and irreversibly deleted.
                
                 *
                 * ### Default
                 *
                 * > ```json
                 * > true
                 * > ```
                 *
                 * ## Selector
                 *
                 * > ```yaml
                 * > boolean: {}
                 * > ```
                 */
        soft_delete?: boolean;
      }) => Promise<void>;
      /**
       * ### Restore soft-deleted entity
       *
       * > Re-enables a previously soft-deleted (disabled) entity and removes it from the trash. The entity will resume normal operation.
       * >
       */
      restore: (service_data: {
        /**
         * ## Entity ID
         *
         * > The entity_id to restore from trash.
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id: PICK_ENTITY | PICK_ENTITY[];
      }) => Promise<void>;
      /**
       * ### Scan for unused entities
       *
       * > Scans the Home Assistant instance for unused entities, automations, scripts, and helpers. Results are saved to storage and fired as the ha_optimizer_scan_complete event.
       * >
       */
      scan: <T = unknown>(service_data: EmptyObject) => Promise<T>;
    };
    // # MARK: hassio
    hassio: {
      /**
       * ### addon_restart
       *
       * >
       */
      addon_restart: (service_data: {
        /**
         * ## addon
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "addon": "core_ssh"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > addon: {}
         * > ```
         */
        addon: (typeof InstalledAddons)[keyof typeof InstalledAddons];
      }) => Promise<void>;
      /**
       * ### addon_start
       *
       * >
       */
      addon_start: (service_data: {
        /**
         * ## addon
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "addon": "core_ssh"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > addon: {}
         * > ```
         */
        addon: (typeof InstalledAddons)[keyof typeof InstalledAddons];
      }) => Promise<void>;
      /**
       * ### addon_stdin
       *
       * >
       */
      addon_stdin: (service_data: {
        /**
         * ## addon
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "addon": "core_ssh"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > addon: {}
         * > ```
         */
        addon: (typeof InstalledAddons)[keyof typeof InstalledAddons];
      }) => Promise<void>;
      /**
       * ### addon_stop
       *
       * >
       */
      addon_stop: (service_data: {
        /**
         * ## addon
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "addon": "core_ssh"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > addon: {}
         * > ```
         */
        addon: (typeof InstalledAddons)[keyof typeof InstalledAddons];
      }) => Promise<void>;
      /**
       * ### app_restart
       *
       * >
       */
      app_restart: (service_data: {
        /**
         * ## app
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "app": "core_ssh"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > app: {}
         * > ```
         */
        app: unknown;
      }) => Promise<void>;
      /**
       * ### app_start
       *
       * >
       */
      app_start: (service_data: {
        /**
         * ## app
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "app": "core_ssh"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > app: {}
         * > ```
         */
        app: unknown;
      }) => Promise<void>;
      /**
       * ### app_stdin
       *
       * >
       */
      app_stdin: (service_data: {
        /**
         * ## app
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "app": "core_ssh"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > app: {}
         * > ```
         */
        app: unknown;
        /**
         * ## input
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        input: Record<string, unknown> | unknown[];
      }) => Promise<void>;
      /**
       * ### app_stop
       *
       * >
       */
      app_stop: (service_data: {
        /**
         * ## app
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "app": "core_ssh"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > app: {}
         * > ```
         */
        app: unknown;
      }) => Promise<void>;
      /**
       * ### backup_full
       *
       * >
       */
      backup_full: <T = unknown>(service_data?: {
        /**
         * ## compressed
         *
         * ### Default
         *
         * > ```json
         * > true
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        compressed?: boolean;
        /**
         * ## homeassistant_exclude_database
         *
         * ### Default
         *
         * > ```json
         * > false
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        homeassistant_exclude_database?: boolean;
        /**
         * ## location
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "location": "my_backup_mount"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > backup_location: {}
         * > ```
         */
        location?: string;
        /**
         * ## name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "name": "Backup 1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        name?: string;
        /**
         * ## password
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "password": "password"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        password?: string;
      }) => Promise<T>;
      /**
       * ### backup_partial
       *
       * >
       */
      backup_partial: <T = unknown>(service_data?: {
        /**
         * ## addons
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "addons": "core_ssh,core_samba,core_mosquitto"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        addons?: Record<string, unknown> | unknown[];
        /**
         * ## apps
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "apps": "core_ssh,core_samba,core_mosquitto"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        apps?: Record<string, unknown> | unknown[];
        /**
         * ## compressed
         *
         * ### Default
         *
         * > ```json
         * > true
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        compressed?: boolean;
        /**
         * ## folders
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "folders": "homeassistant,share"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        folders?: Record<string, unknown> | unknown[];
        /**
         * ## homeassistant
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        homeassistant?: boolean;
        /**
         * ## homeassistant_exclude_database
         *
         * ### Default
         *
         * > ```json
         * > false
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        homeassistant_exclude_database?: boolean;
        /**
         * ## location
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "location": "my_backup_mount"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > backup_location: {}
         * > ```
         */
        location?: string;
        /**
         * ## name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "name": "Partial backup 1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        name?: string;
        /**
         * ## password
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "password": "password"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        password?: string;
      }) => Promise<T>;
      /**
       * ### host_reboot
       *
       * >
       */
      host_reboot: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### host_shutdown
       *
       * >
       */
      host_shutdown: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### mount_reload
       *
       * >
       */
      mount_reload: (service_data: {
        /**
         * ## device_id
         *
         * ## Selector
         *
         * > ```yaml
         * > device:
         * >   filter:
         * >     - integration: hassio
         * >       model: Home Assistant Mount
         * >   multiple: false
         * > ```
         */
        device_id: TDeviceId;
      }) => Promise<void>;
      /**
       * ### restore_full
       *
       * >
       */
      restore_full: (service_data?: {
        /**
         * ## password
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "password": "password"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        password?: string;
        /**
         * ## slug
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        slug: string;
      }) => Promise<void>;
      /**
       * ### restore_partial
       *
       * >
       */
      restore_partial: (service_data?: {
        /**
         * ## addons
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "addons": "core_ssh,core_samba,core_mosquitto"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        addons?: Record<string, unknown> | unknown[];
        /**
         * ## apps
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "apps": "core_ssh,core_samba,core_mosquitto"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        apps?: Record<string, unknown> | unknown[];
        /**
         * ## folders
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "folders": "homeassistant,share"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        folders?: Record<string, unknown> | unknown[];
        /**
         * ## homeassistant
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        homeassistant?: boolean;
        /**
         * ## password
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "password": "password"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        password?: string;
        /**
         * ## slug
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        slug: string;
      }) => Promise<void>;
    };
    // # MARK: homeassistant
    homeassistant: {
      /**
       * ### check_config
       *
       * >
       */
      check_config: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### reload_all
       *
       * >
       */
      reload_all: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### reload_config_entry
       *
       * >
       */
      reload_config_entry: (service_data?: {
        /**
         * ## entry_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entry_id": "8955375327824e14ba89e4b29cc3ec9a"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry: {}
         * > ```
         */
        entry_id?:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
      }) => Promise<void>;
      /**
       * ### reload_core_config
       *
       * >
       */
      reload_core_config: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### reload_custom_templates
       *
       * >
       */
      reload_custom_templates: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### restart
       *
       * >
       */
      restart: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### save_persistent_states
       *
       * >
       */
      save_persistent_states: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### set_location
       *
       * >
       */
      set_location: (service_data?: {
        /**
         * ## elevation
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "elevation": "120"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   mode: box
         * >   step: any
         * > ```
         */
        elevation?: number;
        /**
         * ## latitude
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "latitude": "32.87336"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   mode: box
         * >   min: -90
         * >   max: 90
         * >   step: any
         * > ```
         */
        latitude: number;
        /**
         * ## longitude
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "longitude": "117.22743"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   mode: box
         * >   min: -180
         * >   max: 180
         * >   step: any
         * > ```
         */
        longitude: number;
      }) => Promise<void>;
      /**
       * ### stop
       *
       * >
       */
      stop: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### toggle
       *
       * >
       */
      toggle: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### turn_off
       *
       * >
       */
      turn_off: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### update_entity
       *
       * >
       */
      update_entity: (service_data: {
        /**
         * ## entity_id
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        entity_id: PICK_ENTITY | PICK_ENTITY[];
      }) => Promise<void>;
    };
    // # MARK: humidifier
    humidifier: {
      /**
       * ### set_humidity
       *
       * >
       */
      set_humidity: (
        service_data: {
          /**
           * ## humidity
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 100
           * >   unit_of_measurement: '%'
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          humidity: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - humidifier
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_mode
       *
       * >
       */
      set_mode: (
        service_data: {
          /**
           * ## mode
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "mode": "away"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   attribute: mode
           * >   multiple: false
           * > ```
           */
          mode: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - humidifier
           * >     supported_features:
           * >       - 1
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### toggle
       *
       * >
       */
      toggle: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - humidifier
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_off
       *
       * >
       */
      turn_off: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - humidifier
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - humidifier
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: image
    image: {
      /**
       * ### snapshot
       *
       * >
       */
      snapshot: (
        service_data: {
          /**
           * ## filename
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "filename": "/tmp/image_snapshot.jpg"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          filename: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - image
           * > ```
           */
          entity_id: PICK_ENTITY<"image"> | PICK_ENTITY<"image">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: image_processing
    image_processing: {
      /**
       * ### scan
       *
       * >
       */
      scan: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - image_processing
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: input_boolean
    input_boolean: {
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### toggle
       *
       * >
       */
      toggle: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_boolean
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_off
       *
       * >
       */
      turn_off: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_boolean
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_boolean
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: input_button
    input_button: {
      /**
       * ### press
       *
       * >
       */
      press: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_button
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
    };
    // # MARK: input_datetime
    input_datetime: {
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### set_datetime
       *
       * >
       */
      set_datetime: (
        service_data?: {
          /**
           * ## date
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "date": "\"2019-04-20\""
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          date?: string;
          /**
           * ## datetime
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "datetime": "\"2019-04-20 05:04:20\""
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          datetime?: string;
          /**
           * ## time
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "time": "\"05:04:20\""
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > time: {}
           * > ```
           */
          time?: string;
          /**
           * ## timestamp
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 9223372036854776000
           * >   mode: box
           * >   step: 1
           * > ```
           */
          timestamp?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_datetime
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: input_number
    input_number: {
      /**
       * ### decrement
       *
       * >
       */
      decrement: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_number
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### increment
       *
       * >
       */
      increment: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_number
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### set_value
       *
       * >
       */
      set_value: (
        service_data: {
          /**
           * ## value
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 9223372036854776000
           * >   step: 0.001
           * >   mode: box
           * > ```
           */
          value: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_number
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: input_select
    input_select: {
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### select_first
       *
       * >
       */
      select_first: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_select
           * > ```
           */
          entity_id: PICK_ENTITY<"input_select"> | PICK_ENTITY<"input_select">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### select_last
       *
       * >
       */
      select_last: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_select
           * > ```
           */
          entity_id: PICK_ENTITY<"input_select"> | PICK_ENTITY<"input_select">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### select_next
       *
       * >
       */
      select_next: (
        service_data?: {
          /**
           * ## cycle
           *
           * ### Default
           *
           * > ```json
           * > true
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          cycle?: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_select
           * > ```
           */
          entity_id: PICK_ENTITY<"input_select"> | PICK_ENTITY<"input_select">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### select_option
       *
       * >
       */
      select_option: (
        service_data: {
          /**
           * ## option
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "option": "\"Item A\""
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   hide_states:
           * >     - unavailable
           * >     - unknown
           * >   multiple: false
           * > ```
           */
          option: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_select
           * > ```
           */
          entity_id: PICK_ENTITY<"input_select"> | PICK_ENTITY<"input_select">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### select_previous
       *
       * >
       */
      select_previous: (
        service_data?: {
          /**
           * ## cycle
           *
           * ### Default
           *
           * > ```json
           * > true
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          cycle?: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_select
           * > ```
           */
          entity_id: PICK_ENTITY<"input_select"> | PICK_ENTITY<"input_select">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_options
       *
       * >
       */
      set_options: (
        service_data: {
          /**
           * ## options
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "options": "[\"Item A\", \"Item B\", \"Item C\"]"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: true
           * >   multiline: false
           * > ```
           */
          options: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_select
           * > ```
           */
          entity_id: PICK_ENTITY<"input_select"> | PICK_ENTITY<"input_select">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: input_text
    input_text: {
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### set_value
       *
       * >
       */
      set_value: (
        service_data: {
          /**
           * ## value
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "value": "This is an example text"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          value: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - input_text
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: lawn_mower
    lawn_mower: {
      /**
       * ### dock
       *
       * >
       */
      dock: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lawn_mower
           * >     supported_features:
           * >       - 4
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### pause
       *
       * >
       */
      pause: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lawn_mower
           * >     supported_features:
           * >       - 2
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### start_mowing
       *
       * >
       */
      start_mowing: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lawn_mower
           * >     supported_features:
           * >       - 1
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: light
    light: {
      /**
       * ### toggle
       *
       * >
       */
      toggle: (
        service_data?: {
          /**
           * ## advanced_fields
           *
           * ## Selector
           *
           * > ```yaml
           * >
           * > ```
           */
          advanced_fields?: unknown;
          /**
           * ## brightness_pct
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 100
           * >   unit_of_measurement: '%'
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          brightness_pct?: number;
          /**
           * ## color_temp_kelvin
           *
           * ## Selector
           *
           * > ```yaml
           * > color_temp:
           * >   unit: kelvin
           * >   min: 2000
           * >   max: 6500
           * > ```
           */
          color_temp_kelvin?: number;
          /**
           * ## effect
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   attribute: effect
           * >   multiple: false
           * > ```
           */
          effect?: string;
          /**
           * ## rgb_color
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "rgb_color": "[255, 100, 100]"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > color_rgb: {}
           * > ```
           */
          rgb_color?: [number, number, number];
          /**
           * ## transition
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 300
           * >   unit_of_measurement: seconds
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          transition?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - light
           * > ```
           */
          entity_id: PICK_ENTITY<"light"> | PICK_ENTITY<"light">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_off
       *
       * >
       */
      turn_off: (
        service_data?: {
          /**
           * ## advanced_fields
           *
           * ## Selector
           *
           * > ```yaml
           * >
           * > ```
           */
          advanced_fields?: unknown;
          /**
           * ## transition
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 300
           * >   unit_of_measurement: seconds
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          transition?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - light
           * > ```
           */
          entity_id: PICK_ENTITY<"light"> | PICK_ENTITY<"light">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (
        service_data?: {
          /**
           * ## advanced_fields
           *
           * ## Selector
           *
           * > ```yaml
           * >
           * > ```
           */
          advanced_fields?: unknown;
          /**
           * ## brightness_pct
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 100
           * >   unit_of_measurement: '%'
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          brightness_pct?: number;
          /**
           * ## brightness_step_pct
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: -100
           * >   max: 100
           * >   unit_of_measurement: '%'
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          brightness_step_pct?: number;
          /**
           * ## color_temp_kelvin
           *
           * ## Selector
           *
           * > ```yaml
           * > color_temp:
           * >   unit: kelvin
           * >   min: 2000
           * >   max: 6500
           * > ```
           */
          color_temp_kelvin?: number;
          /**
           * ## effect
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   attribute: effect
           * >   multiple: false
           * > ```
           */
          effect?: string;
          /**
           * ## rgb_color
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "rgb_color": "[255, 100, 100]"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > color_rgb: {}
           * > ```
           */
          rgb_color?: [number, number, number];
          /**
           * ## transition
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 300
           * >   unit_of_measurement: seconds
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          transition?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - light
           * > ```
           */
          entity_id: PICK_ENTITY<"light"> | PICK_ENTITY<"light">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: lock
    lock: {
      /**
       * ### lock
       *
       * >
       */
      lock: (
        service_data?: {
          /**
           * ## code
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "code": "1234"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          code?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lock
           * > ```
           */
          entity_id: PICK_ENTITY<"lock"> | PICK_ENTITY<"lock">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### open
       *
       * >
       */
      open: (
        service_data?: {
          /**
           * ## code
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "code": "1234"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          code?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lock
           * >     supported_features:
           * >       - 1
           * > ```
           */
          entity_id: PICK_ENTITY<"lock"> | PICK_ENTITY<"lock">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### unlock
       *
       * >
       */
      unlock: (
        service_data?: {
          /**
           * ## code
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "code": "1234"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          code?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lock
           * > ```
           */
          entity_id: PICK_ENTITY<"lock"> | PICK_ENTITY<"lock">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: logbook
    logbook: {
      /**
       * ### log
       *
       * >
       */
      log: (service_data?: {
        /**
         * ## domain
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "domain": "light"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        domain?: string;
        /**
         * ## entity_id
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id?: PICK_ENTITY | PICK_ENTITY[];
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "is being used"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        message: string;
        /**
         * ## name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "name": "Kitchen"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        name: string;
      }) => Promise<void>;
    };
    // # MARK: logger
    logger: {
      /**
       * ### set_default_level
       *
       * >
       */
      set_default_level: (service_data?: {
        /**
         * ## level
         */
        level?: "debug" | "info" | "warning" | "error" | "fatal" | "critical";
      }) => Promise<void>;
      /**
       * ### set_level
       *
       * >
       */
      set_level: (service_data: EmptyObject) => Promise<void>;
    };
    // # MARK: mass_queue
    mass_queue: {
      /**
       * ### clear_queue_from_here
       *
       * >
       */
      clear_queue_from_here: (service_data: {
        /**
         * ## Entity
         *
         * > Music Assistant Media Player Entity
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - media_player
         * >   integration: music_assistant
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity:
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
      }) => Promise<void>;
      /**
       * ### get_album
       *
       * >
       */
      get_album: <T = unknown>(service_data: {
        /**
         * ## Config Entry ID
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: mass_queue
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## Album URI
         *
         * > URI for the Album
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "uri": "library://album/12"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        uri: string;
      }) => Promise<T>;
      /**
       * ### get_album_tracks
       *
       * >
       */
      get_album_tracks: <T = unknown>(service_data?: {
        /**
         * ## Config Entry ID
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: mass_queue
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## page
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "page": "0"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 0
         * >   max: 1000
         * >   step: 1
         * >   mode: box
         * > ```
         */
        page?: number;
        /**
         * ## Album URI
         *
         * > URI for the album
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "uri": "library://album/12"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        uri: string;
      }) => Promise<T>;
      /**
       * ### get_artist
       *
       * >
       */
      get_artist: <T = unknown>(service_data: {
        /**
         * ## Config Entry ID
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: mass_queue
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## Artist URI
         *
         * > URI for the artist
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "uri": "library://artist/12"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        uri: string;
      }) => Promise<T>;
      /**
       * ### get_artist_tracks
       *
       * >
       */
      get_artist_tracks: <T = unknown>(service_data?: {
        /**
         * ## Config Entry ID
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: mass_queue
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## page
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "page": "0"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 0
         * >   max: 1000
         * >   step: 1
         * >   mode: box
         * > ```
         */
        page?: number;
        /**
         * ## Artist URI
         *
         * > URI for the artist
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "uri": "library://artist/12"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        uri: string;
      }) => Promise<T>;
      /**
       * ### get_group_volume
       *
       * >
       */
      get_group_volume: <T = unknown>(service_data: {
        /**
         * ## Entity
         *
         * > Music Assistant Media Player Entity
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - media_player
         * >   integration: music_assistant
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity:
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
      }) => Promise<T>;
      /**
       * ### get_playlist
       *
       * >
       */
      get_playlist: <T = unknown>(service_data: {
        /**
         * ## Config Entry ID
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: mass_queue
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## Playlist URI
         *
         * > URI for the playlist
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "uri": "library://playlist/12"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        uri: string;
      }) => Promise<T>;
      /**
       * ### get_playlist_tracks
       *
       * >
       */
      get_playlist_tracks: <T = unknown>(service_data?: {
        /**
         * ## Config Entry ID
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: mass_queue
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## page
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "page": "0"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 0
         * >   max: 1000
         * >   step: 1
         * >   mode: box
         * > ```
         */
        page?: number;
        /**
         * ## Playlist URI
         *
         * > URI for the playlist
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "uri": "library://playlist/12"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        uri: string;
      }) => Promise<T>;
      /**
       * ### get_podcast
       *
       * >
       */
      get_podcast: <T = unknown>(service_data: {
        /**
         * ## Config Entry ID
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: mass_queue
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## Podcast URI
         *
         * > URI for the podcast
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "uri": "library://podcast/12"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        uri: string;
      }) => Promise<T>;
      /**
       * ### get_podcast_episodes
       *
       * >
       */
      get_podcast_episodes: <T = unknown>(service_data: {
        /**
         * ## Config Entry ID
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: mass_queue
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## Podcast URI
         *
         * > URI for the podcast
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "uri": "library://podcast/12"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        uri: string;
      }) => Promise<T>;
      /**
       * ### get_queue_items
       *
       * >
       */
      get_queue_items: <T = unknown>(service_data?: {
        /**
         * ## Entity
         *
         * > Music Assistant Media Player Entity
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - media_player
         * >   integration: music_assistant
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity:
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
        /**
         * ## Limit
         *
         * > Limit on the number of items in queue to return
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "limit": "500"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 1
         * >   max: 1000
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        limit?: number;
        /**
         * ## Limit After
         *
         * > Number of items to pull after current active item in queue.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "limit_after": "50"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > 100
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 1
         * >   max: 1000
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        limit_after?: number;
        /**
         * ## Limit Before
         *
         * > Number of items to pull before current active item in queue.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "limit_before": "5"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > 5
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 1
         * >   max: 1000
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        limit_before?: number;
        /**
         * ## Offset
         *
         * > Location in queue to start where zero equals the first item in queue, not the current item.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "offset": "50"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 1
         * >   max: 1000
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        offset?: number;
      }) => Promise<T>;
      /**
       * ### get_recommendations
       *
       * >
       */
      get_recommendations: <T = unknown>(service_data?: {
        /**
         * ## Entity
         *
         * > Music Assistant Media Player Entity
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - media_player
         * >   integration: music_assistant
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity:
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
        /**
         * ## Providers
         *
         * > Limit recommendations to the specified providers.
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: true
         * >   multiline: false
         * > ```
         */
        providers?: string;
      }) => Promise<T>;
      /**
       * ### move_queue_item_down
       *
       * >
       */
      move_queue_item_down: (service_data: {
        /**
         * ## Entity
         *
         * > Music Assistant Media Player Entity
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - media_player
         * >   integration: music_assistant
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity:
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
        /**
         * ## Queue Item ID
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        queue_item_id: string;
      }) => Promise<void>;
      /**
       * ### move_queue_item_next
       *
       * >
       */
      move_queue_item_next: (service_data: {
        /**
         * ## Entity
         *
         * > Music Assistant Media Player Entity
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - media_player
         * >   integration: music_assistant
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity:
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
        /**
         * ## Queue Item ID
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        queue_item_id: string;
      }) => Promise<void>;
      /**
       * ### move_queue_item_up
       *
       * >
       */
      move_queue_item_up: (service_data: {
        /**
         * ## Entity
         *
         * > Music Assistant Media Player Entity
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - media_player
         * >   integration: music_assistant
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity:
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
        /**
         * ## Queue Item ID
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        queue_item_id: string;
      }) => Promise<void>;
      /**
       * ### play_queue_item
       *
       * >
       */
      play_queue_item: (service_data: {
        /**
         * ## Entity
         *
         * > Music Assistant Media Player Entity
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - media_player
         * >   integration: music_assistant
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity:
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
        /**
         * ## Queue Item ID
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        queue_item_id: string;
      }) => Promise<void>;
      /**
       * ### remove_playlist_tracks
       *
       * >
       */
      remove_playlist_tracks: (service_data: {
        /**
         * ## Config Entry ID
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: mass_queue
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## playlist_id
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        playlist_id: string;
        /**
         * ## positions_to_remove
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: true
         * >   multiline: false
         * > ```
         */
        positions_to_remove: string;
      }) => Promise<void>;
      /**
       * ### remove_queue_item
       *
       * >
       */
      remove_queue_item: (service_data: {
        /**
         * ## Entity
         *
         * > Music Assistant Media Player Entity
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - media_player
         * >   integration: music_assistant
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity:
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
        /**
         * ## Queue Item ID
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        queue_item_id: string;
      }) => Promise<void>;
      /**
       * ### send_command
       *
       * >
       */
      send_command: <T = unknown>(service_data?: {
        /**
         * ## Command
         *
         * > Command to send to Music Assistant
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        command: string;
        /**
         * ## Config Entry ID
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: mass_queue
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## Data
         *
         * > Command data to send
         *
         * ### Default
         *
         * > ```json
         * > {}
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        data?: {};
      }) => Promise<T>;
      /**
       * ### set_group_volume
       *
       * >
       */
      set_group_volume: (service_data: {
        /**
         * ## Entity
         *
         * > Music Assistant Media Player Entity
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - media_player
         * >   integration: music_assistant
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity:
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
        /**
         * ## Volume Level
         *
         * > Volume level to set the player to.
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 1
         * >   max: 100
         * >   step: 1
         * >   unit_of_measurement: '%'
         * >   mode: slider
         * > ```
         */
        volume_level: number;
      }) => Promise<void>;
      /**
       * ### unfavorite_current_item
       *
       * >
       */
      unfavorite_current_item: (service_data: {
        /**
         * ## Entity
         *
         * > Music Assistant Media Player Entity
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - media_player
         * >   integration: music_assistant
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity:
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">
          | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
      }) => Promise<void>;
    };
    // # MARK: matter
    matter: {
      /**
       * ### clear_lock_credential
       *
       * >
       */
      clear_lock_credential: (
        service_data: {
          /**
           * ## credential_index
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 65534
           * >   step: 1
           * >   mode: box
           * > ```
           */
          credential_index: number;
          /**
           * ## credential_type
           */
          credential_type: "pin" | "rfid" | "fingerprint" | "finger_vein" | "face";
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lock
           * >     integration: matter
           * > ```
           */
          entity_id: PICK_FROM_PLATFORM<"matter", "lock"> | PICK_FROM_PLATFORM<"matter", "lock">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### clear_lock_user
       *
       * >
       */
      clear_lock_user: (
        service_data: {
          /**
           * ## user_index
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 1
           * >   max: 65534
           * >   step: 1
           * >   mode: box
           * > ```
           */
          user_index: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lock
           * >     integration: matter
           * > ```
           */
          entity_id: PICK_FROM_PLATFORM<"matter", "lock"> | PICK_FROM_PLATFORM<"matter", "lock">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### get_lock_credential_status
       *
       * >
       */
      get_lock_credential_status: <T = unknown>(
        service_data: {
          /**
           * ## credential_index
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 65534
           * >   step: 1
           * >   mode: box
           * > ```
           */
          credential_index: number;
          /**
           * ## credential_type
           */
          credential_type:
            | "programming_pin"
            | "pin"
            | "rfid"
            | "fingerprint"
            | "finger_vein"
            | "face"
            | "aliro_credential_issuer_key"
            | "aliro_evictable_endpoint_key"
            | "aliro_non_evictable_endpoint_key";
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lock
           * >     integration: matter
           * > ```
           */
          entity_id: PICK_FROM_PLATFORM<"matter", "lock"> | PICK_FROM_PLATFORM<"matter", "lock">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<T>;
      /**
       * ### get_lock_info
       *
       * >
       */
      get_lock_info: <T = unknown>(
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lock
           * >     integration: matter
           * > ```
           */
          entity_id: PICK_FROM_PLATFORM<"matter", "lock"> | PICK_FROM_PLATFORM<"matter", "lock">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<T>;
      /**
       * ### get_lock_users
       *
       * >
       */
      get_lock_users: <T = unknown>(
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lock
           * >     integration: matter
           * > ```
           */
          entity_id: PICK_FROM_PLATFORM<"matter", "lock"> | PICK_FROM_PLATFORM<"matter", "lock">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<T>;
      /**
       * ### set_lock_credential
       *
       * >
       */
      set_lock_credential: <T = unknown>(
        service_data?: {
          /**
           * ## credential_data
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          credential_data: string;
          /**
           * ## credential_index
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 65534
           * >   step: 1
           * >   mode: box
           * > ```
           */
          credential_index?: number;
          /**
           * ## credential_type
           */
          credential_type: "pin" | "rfid" | "fingerprint" | "finger_vein" | "face";
          /**
           * ## user_index
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 1
           * >   max: 65534
           * >   step: 1
           * >   mode: box
           * > ```
           */
          user_index?: number;
          /**
           * ## user_status
           */
          user_status?: "occupied_enabled" | "occupied_disabled";
          /**
           * ## user_type
           */
          user_type?:
            | "unrestricted_user"
            | "year_day_schedule_user"
            | "week_day_schedule_user"
            | "programming_user"
            | "non_access_user"
            | "forced_user"
            | "disposable_user"
            | "expiring_user"
            | "schedule_restricted_user"
            | "remote_only_user";
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lock
           * >     integration: matter
           * > ```
           */
          entity_id: PICK_FROM_PLATFORM<"matter", "lock"> | PICK_FROM_PLATFORM<"matter", "lock">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<T>;
      /**
       * ### set_lock_user
       *
       * >
       */
      set_lock_user: (
        service_data?: {
          /**
           * ## credential_rule
           */
          credential_rule?: "single" | "dual" | "tri";
          /**
           * ## user_index
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 1
           * >   max: 255
           * >   step: 1
           * >   mode: box
           * > ```
           */
          user_index?: number;
          /**
           * ## user_name
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          user_name?: string;
          /**
           * ## user_type
           */
          user_type?:
            | "unrestricted_user"
            | "year_day_schedule_user"
            | "week_day_schedule_user"
            | "programming_user"
            | "non_access_user"
            | "forced_user"
            | "disposable_user"
            | "expiring_user"
            | "schedule_restricted_user"
            | "remote_only_user";
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lock
           * >     integration: matter
           * > ```
           */
          entity_id: PICK_FROM_PLATFORM<"matter", "lock"> | PICK_FROM_PLATFORM<"matter", "lock">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### water_heater_boost
       *
       * >
       */
      water_heater_boost: (
        service_data?: {
          /**
           * ## duration
           *
           * ### Default
           *
           * > ```json
           * > 3600
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 60
           * >   max: 14400
           * >   step: 60
           * >   mode: box
           * > ```
           */
          duration: number;
          /**
           * ## emergency_boost
           *
           * ### Default
           *
           * > ```json
           * > false
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          emergency_boost?: boolean;
          /**
           * ## temporary_setpoint
           *
           * ### Default
           *
           * > ```json
           * > 65
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 30
           * >   max: 65
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          temporary_setpoint?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - water_heater
           * >     integration: matter
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: media_player
    media_player: {
      /**
       * ### browse_media
       *
       * >
       */
      browse_media: <T = unknown>(
        service_data?: {
          /**
           * ## media_content_id
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "media_content_id": "A:ALBUMARTIST/Beatles"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          media_content_id?: string;
          /**
           * ## media_content_type
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "media_content_type": "music"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          media_content_type?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 131072
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<T>;
      /**
       * ### clear_playlist
       *
       * >
       */
      clear_playlist: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 8192
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### join
       *
       * >
       */
      join: (
        service_data: {
          /**
           * ## group_members
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "group_members": "- media_player.multiroom_player2\n- media_player.multiroom_player3\n"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > entity:
           * >   multiple: true
           * >   domain:
           * >     - media_player
           * >   reorder: false
           * > ```
           */
          group_members: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 524288
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### media_next_track
       *
       * >
       */
      media_next_track: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 32
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### media_pause
       *
       * >
       */
      media_pause: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 1
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### media_play
       *
       * >
       */
      media_play: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 16384
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### media_play_pause
       *
       * >
       */
      media_play_pause: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 16385
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### media_previous_track
       *
       * >
       */
      media_previous_track: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 16
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### media_seek
       *
       * >
       */
      media_seek: (
        service_data: {
          /**
           * ## seek_position
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 9223372036854776000
           * >   step: 0.01
           * >   mode: box
           * > ```
           */
          seek_position: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 2
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### media_stop
       *
       * >
       */
      media_stop: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 4096
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### play_media
       *
       * >
       */
      play_media: (
        service_data?: {
          /**
           * ## announce
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "announce": "true"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          announce?: boolean;
          /**
           * ## enqueue
           */
          enqueue?: "play" | "next" | "add" | "replace";
          /**
           * ## media
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "media": "{\"media_content_id\": \"https://home-assistant.io/images/cast/splash.png\", \"media_content_type\": \"music\"}"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > media:
           * >   multiple: false
           * > ```
           */
          media: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 512
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### repeat_set
       *
       * >
       */
      repeat_set: (
        service_data: {
          /**
           * ## repeat
           */
          repeat: "off" | "all" | "one";
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 262144
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### search_media
       *
       * >
       */
      search_media: <T = unknown>(
        service_data?: {
          /**
           * ## media_content_id
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "media_content_id": "A:ALBUMARTIST/Beatles"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          media_content_id?: string;
          /**
           * ## media_content_type
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "media_content_type": "music"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          media_content_type?: string;
          /**
           * ## media_filter_classes
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "media_filter_classes": "album,artist"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: true
           * >   multiline: false
           * > ```
           */
          media_filter_classes?: string;
          /**
           * ## search_query
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "search_query": "Beatles"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          search_query: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 4194304
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<T>;
      /**
       * ### select_sound_mode
       *
       * >
       */
      select_sound_mode: (
        service_data?: {
          /**
           * ## sound_mode
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "sound_mode": "Music"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   attribute: sound_mode
           * >   multiple: false
           * > ```
           */
          sound_mode?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 65536
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### select_source
       *
       * >
       */
      select_source: (
        service_data: {
          /**
           * ## source
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "source": "video1"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   attribute: source
           * >   multiple: false
           * > ```
           */
          source: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 2048
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### shuffle_set
       *
       * >
       */
      shuffle_set: (
        service_data: {
          /**
           * ## shuffle
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          shuffle: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 32768
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### toggle
       *
       * >
       */
      toggle: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 384
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_off
       *
       * >
       */
      turn_off: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 256
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 128
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### unjoin
       *
       * >
       */
      unjoin: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 524288
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### volume_down
       *
       * >
       */
      volume_down: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 4
           * >       - 1024
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### volume_mute
       *
       * >
       */
      volume_mute: (
        service_data: {
          /**
           * ## is_volume_muted
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          is_volume_muted: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 8
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### volume_set
       *
       * >
       */
      volume_set: (
        service_data: {
          /**
           * ## volume_level
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 1
           * >   step: 0.01
           * >   mode: slider
           * > ```
           */
          volume_level: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 4
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### volume_up
       *
       * >
       */
      volume_up: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     supported_features:
           * >       - 4
           * >       - 1024
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: mqtt
    mqtt: {
      /**
       * ### dump
       *
       * >
       */
      dump: (service_data?: {
        /**
         * ## duration
         *
         * ### Default
         *
         * > ```json
         * > 5
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 1
         * >   max: 300
         * >   unit_of_measurement: seconds
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        duration?: number;
        /**
         * ## topic
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "topic": "OpenZWave/#"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        topic?: string;
      }) => Promise<void>;
      /**
       * ### publish
       *
       * >
       */
      publish: (service_data?: {
        /**
         * ## evaluate_payload
         *
         * ### Default
         *
         * > ```json
         * > false
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        evaluate_payload?: boolean;
        /**
         * ## payload
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "payload": "The temperature is {{ states('sensor.temperature') }}"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > template: {}
         * > ```
         */
        payload?: string;
        /**
         * ## qos
         *
         * ### Default
         *
         * > ```json
         * > 0
         * > ```
         */
        qos?: "0" | "1" | "2";
        /**
         * ## retain
         *
         * ### Default
         *
         * > ```json
         * > false
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        retain?: boolean;
        /**
         * ## topic
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "topic": "/homeassistant/hello"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        topic: string;
      }) => Promise<void>;
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
    };
    // # MARK: music_assistant
    music_assistant: {
      /**
       * ### get_library
       *
       * >
       */
      get_library: <T = unknown>(service_data?: {
        /**
         * ## album_artists_only
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "album_artists_only": "true"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > false
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        album_artists_only?: boolean;
        /**
         * ## album_type
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "album_type": "single"
         * > }
         * > ```
         */
        album_type?: ("album" | "single" | "compilation" | "ep" | "unknown")[];
        /**
         * ## config_entry_id
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: music_assistant
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## favorite
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "favorite": "true"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > false
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        favorite?: boolean;
        /**
         * ## limit
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "limit": "25"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > 25
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 1
         * >   max: 500
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        limit?: number;
        /**
         * ## media_type
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "media_type": "playlist"
         * > }
         * > ```
         */
        media_type: "artist" | "album" | "audiobook" | "playlist" | "podcast" | "track" | "radio";
        /**
         * ## offset
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "offset": "25"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > 0
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 1
         * >   max: 1000000
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        offset?: number;
        /**
         * ## order_by
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "order_by": "random"
         * > }
         * > ```
         */
        order_by?:
          | "name"
          | "name_desc"
          | "sort_name"
          | "sort_name_desc"
          | "timestamp_added"
          | "timestamp_added_desc"
          | "last_played"
          | "last_played_desc"
          | "play_count"
          | "play_count_desc"
          | "year"
          | "year_desc"
          | "position"
          | "position_desc"
          | "artist_name"
          | "artist_name_desc"
          | "random"
          | "random_play_count";
        /**
         * ## search
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "search": "We Are The Champions"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        search?: string;
      }) => Promise<T>;
      /**
       * ### get_queue
       *
       * >
       */
      get_queue: <T = unknown>(
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     integration: music_assistant
           * >     supported_features:
           * >       - 512
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"music_assistant", "media_player">
            | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<T>;
      /**
       * ### play_announcement
       *
       * >
       */
      play_announcement: (
        service_data?: {
          /**
           * ## announce_volume
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "announce_volume": "75"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 1
           * >   max: 100
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          announce_volume?: number;
          /**
           * ## pre_announce_url
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "pre_announce_url": "http://someremotesite.com/chime.mp3"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          pre_announce_url?: string;
          /**
           * ## url
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "url": "http://someremotesite.com/doorbell.mp3"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          url: string;
          /**
           * ## use_pre_announce
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "use_pre_announce": "true"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          use_pre_announce?: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     integration: music_assistant
           * >     supported_features:
           * >       - 512
           * >       - 1048576
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"music_assistant", "media_player">
            | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### play_media
       *
       * >
       */
      play_media: (
        service_data?: {
          /**
           * ## album
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "album": "News of the world"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          album?: string;
          /**
           * ## artist
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "artist": "Queen"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          artist?: string;
          /**
           * ## enqueue
           */
          enqueue?: "play" | "replace" | "next" | "replace_next" | "add";
          /**
           * ## media_id
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "media_id": "spotify://playlist/aabbccddeeff"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > object:
           * >   multiple: false
           * > ```
           */
          media_id: Record<string, unknown> | unknown[];
          /**
           * ## media_type
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "media_type": "playlist"
           * > }
           * > ```
           */
          media_type?:
            | "artist"
            | "album"
            | "audiobook"
            | "folder"
            | "playlist"
            | "podcast"
            | "track"
            | "radio";
          /**
           * ## radio_mode
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          radio_mode?: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     integration: music_assistant
           * >     supported_features:
           * >       - 512
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"music_assistant", "media_player">
            | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### search
       *
       * >
       */
      search: <T = unknown>(service_data?: {
        /**
         * ## album
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "album": "News of the world"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        album?: string;
        /**
         * ## artist
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "artist": "Queen"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        artist?: string;
        /**
         * ## config_entry_id
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: music_assistant
         * > ```
         */
        config_entry_id:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## library_only
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "library_only": "true"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > false
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        library_only?: boolean;
        /**
         * ## limit
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "limit": "25"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > 5
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 1
         * >   max: 100
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        limit?: number;
        /**
         * ## media_type
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "media_type": "playlist"
         * > }
         * > ```
         */
        media_type?: (
          | "artist"
          | "album"
          | "audiobook"
          | "playlist"
          | "podcast"
          | "track"
          | "radio"
        )[];
        /**
         * ## name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "name": "We Are The Champions"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        name: string;
      }) => Promise<T>;
      /**
       * ### transfer_queue
       *
       * >
       */
      transfer_queue: (
        service_data?: {
          /**
           * ## auto_play
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "auto_play": "true"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          auto_play?: boolean;
          /**
           * ## source_player
           *
           * ## Selector
           *
           * > ```yaml
           * > entity:
           * >   domain:
           * >     - media_player
           * >   integration: music_assistant
           * >   reorder: false
           * >   multiple: false
           * > ```
           */
          source_player?:
            | PICK_FROM_PLATFORM<"music_assistant", "media_player">
            | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * >     integration: music_assistant
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"music_assistant", "media_player">
            | PICK_FROM_PLATFORM<"music_assistant", "media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: notify
    notify: {
      /**
       * ### Send a notification with alexa_media
       *
       * > Sends a notification message using the alexa_media service.
       */
      alexa_media: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification via alexa_media_bathroom_2
       *
       * > Sends a notification message using the alexa_media_bathroom_2 integration.
       */
      alexa_media_bathroom_2: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification via alexa_media_bedroom_sonos_one
       *
       * > Sends a notification message using the alexa_media_bedroom_sonos_one integration.
       */
      alexa_media_bedroom_sonos_one: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification via alexa_media_bedroom_speaker
       *
       * > Sends a notification message using the alexa_media_bedroom_speaker integration.
       */
      alexa_media_bedroom_speaker: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification via alexa_media_ben_s_2nd_sonos_one_second_edition
       *
       * > Sends a notification message using the alexa_media_ben_s_2nd_sonos_one_second_edition integration.
       */
      alexa_media_ben_s_2nd_sonos_one_second_edition: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification via alexa_media_last_called
       *
       * > Sends a notification message using the alexa_media_last_called integration.
       */
      alexa_media_last_called: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification via alexa_media_living_room_2
       *
       * > Sends a notification message using the alexa_media_living_room_2 integration.
       */
      alexa_media_living_room_2: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification via alexa_media_living_room_sonos_2
       *
       * > Sends a notification message using the alexa_media_living_room_sonos_2 integration.
       */
      alexa_media_living_room_sonos_2: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification via alexa_media_living_room_sonos_one
       *
       * > Sends a notification message using the alexa_media_living_room_sonos_one integration.
       */
      alexa_media_living_room_sonos_one: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification via alexa_media_sonos_arc_ultra
       *
       * > Sends a notification message using the alexa_media_sonos_arc_ultra integration.
       */
      alexa_media_sonos_arc_ultra: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification via alexa_media_this_device
       *
       * > Sends a notification message using the alexa_media_this_device integration.
       */
      alexa_media_this_device: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification with bwainwright28_gmail_com
       *
       * > Sends a notification message using the bwainwright28_gmail_com service.
       */
      bwainwright28_gmail_com: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification via mobile_app_apple_tv
       *
       * > Sends a notification message using the mobile_app_apple_tv integration.
       */
      mobile_app_apple_tv: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification via mobile_app_bens_imac_pro
       *
       * > Sends a notification message using the mobile_app_bens_imac_pro integration.
       */
      mobile_app_bens_imac_pro: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification via mobile_app_bens_iphone
       *
       * > Sends a notification message using the mobile_app_bens_iphone integration.
       */
      mobile_app_bens_iphone: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification via mobile_app_bens_phone
       *
       * > Sends a notification message using the mobile_app_bens_phone integration.
       */
      mobile_app_bens_phone: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### Send a notification with notify
       *
       * > Sends a notification message using the notify service.
       */
      notify: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### persistent_notification
       *
       * >
       */
      persistent_notification: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        data?: Record<string, unknown> | unknown[];
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        message: string;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### send_message
       *
       * >
       */
      send_message: (
        service_data?: {
          /**
           * ## message
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          message: string;
          /**
           * ## title
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          title?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - notify
           * > ```
           */
          entity_id: PICK_ENTITY<"notify"> | PICK_ENTITY<"notify">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### Send a notification with tv
       *
       * > Sends a notification message using the tv service.
       */
      tv: (service_data?: {
        /**
         * ## data
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "data": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        data?: NotificationData & (AndroidNotificationData | AppleNotificationData);
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "The garage door has been open for 10 minutes."
         * > }
         * > ```
         */
        message: string;
        /**
         * ## target
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "target": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        target?: unknown;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Your Garage Door Friend"
         * > }
         * > ```
         */
        title?: string;
      }) => Promise<void>;
    };
    // # MARK: number
    number: {
      /**
       * ### set_value
       *
       * >
       */
      set_value: (
        service_data: {
          /**
           * ## value
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "value": "42"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          value: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - number
           * > ```
           */
          entity_id: PICK_ENTITY<"number"> | PICK_ENTITY<"number">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: openai_conversation
    openai_conversation: {
      /**
       * ### generate_content
       *
       * >
       */
      generate_content: <T = unknown>(service_data?: {
        /**
         * ## config_entry
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: openai_conversation
         * > ```
         */
        config_entry:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## filenames
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "filenames": "- /path/to/file1.txt\n- /path/to/file2.txt\n"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiline: true
         * >   multiple: false
         * > ```
         */
        filenames?: string;
        /**
         * ## prompt
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "prompt": "Hello, how can I help you?"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiline: true
         * >   multiple: false
         * > ```
         */
        prompt: string;
      }) => Promise<T>;
      /**
       * ### generate_image
       *
       * >
       */
      generate_image: <T = unknown>(service_data?: {
        /**
         * ## config_entry
         *
         * ## Selector
         *
         * > ```yaml
         * > config_entry:
         * >   integration: openai_conversation
         * > ```
         */
        config_entry:
          | "01KGNB1SM9Q5V950Y0CGGYJ87D"
          | "01KGNB1SVBWDW2MXPH647M1HER"
          | "01KGNB1SVJRGREE6JMR645H58Y"
          | "01KGNB1T2FM0NT8R23MXD4PNZ4"
          | "01KGNB1W73VP590M9DEP49C16A"
          | "01KGNB1X9QWNJJ2B2DFM0F6HQ8"
          | "01KGNB4WB89SBTYRTA7CRC1CNE"
          | "01KGNB68Y8B88TF39H0FMD0854"
          | "01KGNB692NWHW6V4JX7PCA7T8D"
          | "01KGNB692P1T3MYR9SP5KABMVF"
          | "01KGNEJ1VW5W0MBQQFBVT56DY1"
          | "01KGNFZ1C5N8X5PPGFPJV8ATW3"
          | "01KGNHDASVCXZRP69JW51W7AWT"
          | "01KGNK3DFHE9DF4CJ7WBM8V01Y"
          | "01KGP9Z70JSRJCRV4T3V0GX4G0"
          | "01KGPBGFYYHJ2B2K0WW8PJ27BZ"
          | "01KGPDQQB2C6ZWJ9HPN4BZSVW3"
          | "01KGPDST4QFKXS0PZKN00QK01M"
          | "01KGPDTX6H3W2C727F7DRH0TET"
          | "01KGPDV5H7N3HF1RZNSDFHTB7A"
          | "01KGQ2PXFHSBENFKYFR4QYRMFZ"
          | "01KGQ2Q02S1VPGDZNDDE24A1YM"
          | "01KGQ31C7SJ84Q4DACFK99STK6"
          | "01KGQ33T2WVF93PC45JP1DWHY4"
          | "01KGQ3DRCV68YGMKKNTS1RNPBM"
          | "01KGQ5GQ9JA6ZDH9NBG0A9QDZX"
          | "01KGQ5GS3WWW437ZWZZFBPBXN4"
          | "01KGQ81R7DCYNP5KX4SFGAWVB6"
          | "01KGQ8JRQWYNZES8JQA8250S97"
          | "01KGQANDBGAPKSV8W2PPJ8JCMX"
          | "01KGQAR09X8VNMTE6F55440WPX"
          | "01KGQJ8YCWV1J45SFQ1ACGK5M2"
          | "01KGQJ9A2ZSEGQ0GDETS9YA5EV"
          | "01KGQJYD31F0M99SWDVT2JW5PS"
          | "01KGQNW1RQ05GTVV2A0X562VBR"
          | "01KGQT8ZJWTVX9DXC1KEFG8FG8"
          | "01KGQX5ESRK2J8G1MFYXQ9M68B"
          | "01KGSVRH1C4W71KKZYMTZBT2HP"
          | "01KGVV88BYMVDGDK15VG0D9KYR"
          | "01KGVWA098JKVPMQCCR0G9DQQG"
          | "01KGWE7QF985TGXFP8JS847MCQ"
          | "01KGWJE8X722GR755ZKJA81EY8"
          | "01KGYWMBMEHV3XRQMZB4J0DCKZ"
          | "01KGYWMM5XZWJWRJ1TQ9B1FSM3"
          | "01KH1RG56MRQGSD9V2WA52RZ4N"
          | "01KH1SH05457MYPF8BZMY7MFDJ"
          | "01KHNBWD0MWEFFNEY8K217WMHV"
          | "01KHPJGT8R803RDRPBFA88EDSD"
          | "01KHPR32WETT9ZK6JJ42TE5E4R"
          | "01KHS1NC5XFR2Y6Z0HZN9R2Q45"
          | "01KHS7G1H795CQG23VH10AW6SQ"
          | "01KHS8YSXFZND22GQDTRBQQX61"
          | "01KHSBJSBXNP6NVHKYGTB5TVA9"
          | "01KHSC2S8WVH6QX7EWTDH6XETT"
          | "01KHSCHHDZCRACHJQ57Y63R36Z"
          | "01KJ9S4ETKZFSY2WFTT0HNYB7D"
          | "01KM3MP2KJYCZGP3PJTES7BDMD"
          | "01KM3MZPA0ECG3JBH1DKQ3GVBD"
          | "01KM5JA5KW7WHTBP9DBS24W9CM"
          | "01KM5JQ5DGWDX8JGPZMNPNQN75"
          | "01KM5ZV4PJ58R66HNJFD7V5QRT"
          | "01KMD2PC09MXTCKD1M9ZT6BWXA"
          | "01KMD2PWCAB1338ANZZRJKQ537"
          | "01KMD7MWT455NT69ZBM3AZ01X6"
          | "01KMD83K8HRCEC67NHH7E4YZCH"
          | "01KMD848YYA5VQ93CCJY5FKV1P"
          | "01KMD850RVZ6JZ4SC7AQQ69142"
          | "01KMD860B4PBG51118FCX4Y3Q3"
          | "01KMD86FTGQA0VYEYHV1T749QC"
          | "01KMD878QAYPFV7X2VM2S4KRP0"
          | "01KMD87DGPH9QK40VGJ4PMDXVE"
          | "01KMD87N8CPDAWP2306AV7XWYK"
          | "01KMD87WDZRQRGGEQEY2ZV4JF7"
          | "01KMD8878MBDK8Q7DFZ3TCJ290"
          | "01KMD88F5M6PC4ZK0HGEK9VRM2"
          | "01KMD88TDMRRV6ADSTMENSG2PX"
          | "01KMD891JC9CA50H85Q9PBFFHJ"
          | "01KMD894DJ32A7ACZD47BW7C3K"
          | "01KMD89ATBVMBXPVAXJN3DVMTE"
          | "01KMD89GAAZY5244QKRCEXBHF2"
          | "01KMD89P3Y5AWHQFDN3PH1D1JJ"
          | "01KMD89XDJDRM0NN3MT8Y9SG92"
          | "01KMD8A2ABE7VZXGRV3HETRWXK"
          | "01KMD8AE0S7TEC6AAN46TMFY7X"
          | "01KMD8ANXH68XH6HVX3FAEFVK8"
          | "01KMD8AVGTK1J14VW8Z8KAFXJD"
          | "01KMD8B640PPV194FNENGHJARW"
          | "01KMD8BE3YX3HB7W4HJJZVMBT8"
          | "01KMD8BG03JX6RK7ADXVDM2XEF"
          | "01KMD8BHFMY44FWEY51VZ2H9XC"
          | "01KMD8BKPZ8CG711JWW78T0QCG"
          | "01KMD8BPACH27XM12334EGQPA7"
          | "01KMD8BR272TP391K74K14WRR6"
          | "01KMD8BSREXQV0YE9QYZRQSD3G"
          | "01KMD8BVNCW6R7EKQ7FMAXTD97"
          | "01KMD8BXCYFTRWDCR1QYZZ7GXD"
          | "01KMGTF5TGYDXD50T0R8FG5D90"
          | "01KMGVR9283AKCQKTHX3NY3ACG"
          | "01KMGVV2ZJ67SG5YSJB55NAXMW"
          | "01KMJ7X01SE2VDJAPVRDYK7QRC"
          | "01KMKAK76QR7ZV40AERG9089JC"
          | "01KPDG4JTMWY9CJ8NWAERHSSA1"
          | "01KPDT1865K7GZBMS40ASQ7E9M"
          | "01KPZDRCGCWF4MRXAPPR0792T4"
          | "01KQCCRED7ARHT0K82BTE6RQR5"
          | "01KQN2WH8WXGQVG9PAK7VV1E6Q"
          | "01KQT8R22JSDV1KMEHS7RXBMG8"
          | "01KQT8R86HY4EPBFBDA8HGKZME"
          | "01KQT8RD0TX70AA9C19RP27M5A"
          | "01KQT8RH61JTJDQSQYMVHCRCJM"
          | "01KQT8ZE49TEHPTAMAFW2C746D"
          | "01KQTGZAPXS26DABNJEXSM8GB4"
          | "01KQTGZN9EY7DNK5CJSH5QZDEX"
          | "01KQTKJFWS4P3VPY0QE8EWFA12"
          | "01KQVEG9XZD0YTY92FAAKY0GK6"
          | "01KQY5GJPGNP2NZGNNPB0KG2PP"
          | "01KQY5GJWYSWHZ6ZY9EVC8KWMZ"
          | "01KQZNCVAS42GPXTJG5T98RDTB";
        /**
         * ## prompt
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiline: true
         * >   multiple: false
         * > ```
         */
        prompt: string;
        /**
         * ## quality
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "quality": "standard"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > "standard"
         * > ```
         */
        quality?: "standard" | "hd";
        /**
         * ## size
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "size": "1024x1024"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > "1024x1024"
         * > ```
         */
        size?: "1024x1024" | "1024x1792" | "1792x1024";
        /**
         * ## style
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "style": "vivid"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > "vivid"
         * > ```
         */
        style?: "vivid" | "natural";
      }) => Promise<T>;
    };
    // # MARK: openai_tts
    openai_tts: {
      /**
       * ### OpenAI TTS Speak
       *
       * > Use the OpenAI TTS engine to speak a message with optional features.
       */
      say: (
        service_data?: {
          /**
           * ## Play Chime
           *
           * > * Disables streaming (post-processing required). Play a chime sound before speaking. Falls back to the profile setting when omitted.
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          chime?: boolean;
          /**
           * ## Chime Sound
           *
           * > * Disables streaming (post-processing required). Filename of the chime sound to play.
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "chime_sound": "threetone.mp3"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          chime_sound?: string;
          /**
           * ## Extra Payload
           *
           * > Additional JSON payload to merge with the API request. Useful for custom TTS backends that need extra parameters.
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "extra_payload": "{\"temperature\": 0.7}"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiline: true
           * >   multiple: false
           * > ```
           */
          extra_payload?: string;
          /**
           * ## Instructions
           *
           * > Speaking style instructions for OpenAI gpt-4o-mini-tts model (e.g., "speak like a pirate"). May not be supported by custom backends.
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "instructions": "Speak with enthusiasm"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiline: true
           * >   multiple: false
           * > ```
           */
          instructions?: string;
          /**
           * ## Language
           *
           * > Language code for the speech.
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "language": "en"
           * > }
           * > ```
           *
           * ### Default
           *
           * > ```json
           * > "en"
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          language?: string;
          /**
           * ## Message
           *
           * > The text to be spoken.
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "message": "Hello, this is a test message."
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          message: string;
          /**
           * ## Normalize Audio
           *
           * > * Disables streaming (post-processing required). Apply loudness normalization. Falls back to the profile setting when omitted.
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          normalize_audio?: boolean;
          /**
           * ## Pause Playback
           *
           * > Pause any currently playing media during announcement. Falls back to the profile setting when omitted.
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          pause_playback?: boolean;
          /**
           * ## Speed
           *
           * > Speed of the speech (0.25-4.0). 1.0 is normal speed.
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "speed": "1"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0.25
           * >   max: 4
           * >   step: 0.05
           * >   mode: slider
           * > ```
           */
          speed?: number;
          /**
           * ## TTS Entity
           *
           * > The TTS entity to use for text-to-speech.
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "tts_entity": "tts.openai_tts_tts_1"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > entity:
           * >   domain:
           * >     - tts
           * >   integration: openai_tts
           * >   reorder: false
           * >   multiple: false
           * > ```
           */
          tts_entity:
            | PICK_FROM_PLATFORM<"openai_tts", "tts">
            | PICK_FROM_PLATFORM<"openai_tts", "tts">[];
          /**
           * ## Voice
           *
           * > Voice to use for speech generation (overrides entity default). Voices marked with · gpt-4o-mini-tts only are exclusive to the gpt-4o-mini-tts model and will be rejected by tts-1 / tts-1-hd. For custom TTS backends, you can type any voice name.
           */
          voice?: LiteralUnion<
            | "alloy"
            | "ash"
            | "ballad"
            | "cedar"
            | "coral"
            | "echo"
            | "fable"
            | "marin"
            | "nova"
            | "onyx"
            | "sage"
            | "shimmer"
            | "verse",
            string
          >;
          /**
           * ## Volume
           *
           * > Override speaker volume for this announcement (0.0-1.0).
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 1
           * >   step: 0.05
           * >   mode: slider
           * > ```
           */
          volume?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - media_player
           * > ```
           */
          entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: persistent_notification
    persistent_notification: {
      /**
       * ### create
       *
       * >
       */
      create: (service_data?: {
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "Please check your configuration.yaml."
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        message: string;
        /**
         * ## notification_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "notification_id": "1234"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        notification_id?: string;
        /**
         * ## title
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "title": "Test notification"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        title?: string;
      }) => Promise<void>;
      /**
       * ### dismiss
       *
       * >
       */
      dismiss: (service_data: {
        /**
         * ## notification_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "notification_id": "1234"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        notification_id: string;
      }) => Promise<void>;
      /**
       * ### dismiss_all
       *
       * >
       */
      dismiss_all: (service_data: EmptyObject) => Promise<void>;
    };
    // # MARK: person
    person: {
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
    };
    // # MARK: plant
    plant: {
      /**
       * ### replace_sensor
       *
       * > Replaces an external sensor
       */
      replace_sensor: (service_data?: {
        /**
         * ## Meter entity
         *
         * > The meter entity to replace the sensor for
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "meter_entity": "plant.my_plant_current_temperature"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - sensor
         * >   integration: plant
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        meter_entity:
          | PICK_FROM_PLATFORM<"plant", "sensor">
          | PICK_FROM_PLATFORM<"plant", "sensor">[];
        /**
         * ## Sensor entity
         *
         * > Entity id of the new sensor. Leave blank to remove sensor.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "new_sensor": "sensor.new_temperature_outdoor"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   domain:
         * >     - sensor
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        new_sensor?: PICK_ENTITY<"sensor"> | PICK_ENTITY<"sensor">[];
      }) => Promise<void>;
    };
    // # MARK: powercalc
    powercalc: {
      /**
       * ### Activate playbook
       *
       * > Activate playbook
       */
      activate_playbook: (
        service_data: {
          /**
           * ## Playbook
           *
           * > Playbook identifier
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "playbook_id": "program1"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          playbook_id: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - sensor
           * >     integration: powercalc
           * >     device_class:
           * >       - power
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"powercalc", "sensor">
            | PICK_FROM_PLATFORM<"powercalc", "sensor">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### Calibrate energy sensor
       *
       * > Sets the energy sensor to a given kWh value.
       */
      calibrate_energy: (
        service_data: {
          /**
           * ## Value
           *
           * > Value to which set the meter
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "value": "100"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          value: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - sensor
           * >     device_class:
           * >       - energy
           * > ```
           */
          entity_id: PICK_ENTITY<"sensor"> | PICK_ENTITY<"sensor">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### Calibrate utility meter
       *
       * > Calibrates a utility meter sensor.
       */
      calibrate_utility_meter: (
        service_data: {
          /**
           * ## Value
           *
           * > Value to which set the meter
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "value": "100"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          value: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - sensor
           * > ```
           */
          entity_id: PICK_ENTITY<"sensor"> | PICK_ENTITY<"sensor">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### Change GUI config
       *
       * > Batch change configuration of all Powercalc config entries
       */
      change_gui_config: (service_data: {
        /**
         * ## Field
         */
        field:
          | "create_energy_sensor"
          | "create_utility_meters"
          | "ignore_unavailable_state"
          | "energy_integration_method";
        /**
         * ## Value
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        value: string;
      }) => Promise<void>;
      /**
       * ### get_active_playbook
       *
       * >
       */
      get_active_playbook: <T = unknown>(service_data: EmptyObject) => Promise<T>;
      /**
       * ### Get group entities
       *
       * > Get child entities of a Powercalc power or energy group
       */
      get_group_entities: <T = unknown>(
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - sensor
           * >     integration: powercalc
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"powercalc", "sensor">
            | PICK_FROM_PLATFORM<"powercalc", "sensor">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<T>;
      /**
       * ### Increase daily energy sensor
       *
       * > Increases the sensor with a given amount
       */
      increase_daily_energy: (
        service_data: {
          /**
           * ## Value
           *
           * > Amount to add to the sensor
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "value": "100"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          value: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - sensor
           * >     integration: powercalc
           * >     device_class:
           * >       - energy
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"powercalc", "sensor">
            | PICK_FROM_PLATFORM<"powercalc", "sensor">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### Reload config entries
       *
       * > Reload all Powercalc config entries
       */
      reload: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### Reset energy sensor
       *
       * > Reset an energy sensor to zero kWh
       */
      reset_energy: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - sensor
           * >     device_class:
           * >       - energy
           * > ```
           */
          entity_id: PICK_ENTITY<"sensor"> | PICK_ENTITY<"sensor">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### Stop playbook
       *
       * > Stop active playbook
       */
      stop_playbook: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - sensor
           * >     integration: powercalc
           * >     device_class:
           * >       - power
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"powercalc", "sensor">
            | PICK_FROM_PLATFORM<"powercalc", "sensor">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### Switch to another sub profile
       *
       * > Some profiles in the library has different sub profiles. This service allows you to switch to another one.
       */
      switch_sub_profile: (
        service_data: {
          /**
           * ## Sub profile
           *
           * > Define one of the possible sub profiles
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "profile": "night_vision"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          profile: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - sensor
           * >     integration: powercalc
           * >     device_class:
           * >       - power
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"powercalc", "sensor">
            | PICK_FROM_PLATFORM<"powercalc", "sensor">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### Update profile library
       *
       * > Update the profile library and reinitialize discovery
       */
      update_library: (service_data: EmptyObject) => Promise<void>;
    };
    // # MARK: recorder
    recorder: {
      /**
       * ### disable
       *
       * >
       */
      disable: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### enable
       *
       * >
       */
      enable: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### get_statistics
       *
       * >
       */
      get_statistics: <T = unknown>(service_data?: {
        /**
         * ## end_time
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "end_time": "2025-01-02 00:00:00"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > datetime: {}
         * > ```
         */
        end_time?: string;
        /**
         * ## period
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "period": "hour"
         * > }
         * > ```
         */
        period: "5minute" | "hour" | "day" | "week" | "month" | "year";
        /**
         * ## start_time
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "start_time": "2025-01-01 00:00:00"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > datetime: {}
         * > ```
         */
        start_time: string;
        /**
         * ## statistic_ids
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "statistic_ids": "sensor.energy_consumption,sensor.temperature"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > statistic:
         * >   multiple: true
         * > ```
         */
        statistic_ids: string[];
        /**
         * ## types
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "types": "mean,sum"
         * > }
         * > ```
         */
        types: ("change" | "last_reset" | "max" | "mean" | "min" | "state" | "sum")[];
        /**
         * ## units
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "units": "[object Object]"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        units?: Record<string, unknown> | unknown[];
      }) => Promise<T>;
      /**
       * ### purge
       *
       * >
       */
      purge: (service_data?: {
        /**
         * ## apply_filter
         *
         * ### Default
         *
         * > ```json
         * > false
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        apply_filter?: boolean;
        /**
         * ## keep_days
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 0
         * >   max: 365
         * >   unit_of_measurement: days
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        keep_days?: number;
        /**
         * ## repack
         *
         * ### Default
         *
         * > ```json
         * > false
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        repack?: boolean;
      }) => Promise<void>;
      /**
       * ### purge_entities
       *
       * >
       */
      purge_entities: (service_data?: {
        /**
         * ## domains
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "domains": "sun"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        domains?: Record<string, unknown> | unknown[];
        /**
         * ## entity_globs
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entity_globs": "domain*.object_id*"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        entity_globs?: Record<string, unknown> | unknown[];
        /**
         * ## entity_id
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        entity_id?: PICK_ENTITY | PICK_ENTITY[];
        /**
         * ## keep_days
         *
         * ### Default
         *
         * > ```json
         * > 0
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 0
         * >   max: 365
         * >   unit_of_measurement: days
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        keep_days?: number;
      }) => Promise<void>;
    };
    // # MARK: remote
    remote: {
      /**
       * ### delete_command
       *
       * >
       */
      delete_command: (
        service_data?: {
          /**
           * ## command
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "command": "Mute"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > object:
           * >   multiple: false
           * > ```
           */
          command: Record<string, unknown> | unknown[];
          /**
           * ## device
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "device": "television"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          device?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - remote
           * > ```
           */
          entity_id: PICK_ENTITY<"remote"> | PICK_ENTITY<"remote">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### learn_command
       *
       * >
       */
      learn_command: (
        service_data?: {
          /**
           * ## alternative
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          alternative?: boolean;
          /**
           * ## command
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "command": "Turn on"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > object:
           * >   multiple: false
           * > ```
           */
          command?: Record<string, unknown> | unknown[];
          /**
           * ## command_type
           *
           * ### Default
           *
           * > ```json
           * > "ir"
           * > ```
           */
          command_type?: "ir" | "rf";
          /**
           * ## device
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "device": "television"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          device?: string;
          /**
           * ## timeout
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 60
           * >   step: 5
           * >   unit_of_measurement: seconds
           * >   mode: slider
           * > ```
           */
          timeout?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - remote
           * > ```
           */
          entity_id: PICK_ENTITY<"remote"> | PICK_ENTITY<"remote">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### send_command
       *
       * >
       */
      send_command: (
        service_data?: {
          /**
           * ## command
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "command": "Play"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > object:
           * >   multiple: false
           * > ```
           */
          command: Record<string, unknown> | unknown[];
          /**
           * ## delay_secs
           *
           * ### Default
           *
           * > ```json
           * > 0.4
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 60
           * >   step: 0.1
           * >   unit_of_measurement: seconds
           * >   mode: slider
           * > ```
           */
          delay_secs?: number;
          /**
           * ## device
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "device": "32756745"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          device?: string;
          /**
           * ## hold_secs
           *
           * ### Default
           *
           * > ```json
           * > 0
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 60
           * >   step: 0.1
           * >   unit_of_measurement: seconds
           * >   mode: slider
           * > ```
           */
          hold_secs?: number;
          /**
           * ## num_repeats
           *
           * ### Default
           *
           * > ```json
           * > 1
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 255
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          num_repeats?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - remote
           * > ```
           */
          entity_id: PICK_ENTITY<"remote"> | PICK_ENTITY<"remote">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### toggle
       *
       * >
       */
      toggle: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - remote
           * > ```
           */
          entity_id: PICK_ENTITY<"remote"> | PICK_ENTITY<"remote">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_off
       *
       * >
       */
      turn_off: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - remote
           * > ```
           */
          entity_id: PICK_ENTITY<"remote"> | PICK_ENTITY<"remote">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (
        service_data?: {
          /**
           * ## activity
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "activity": "BedroomTV"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          activity?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - remote
           * > ```
           */
          entity_id: PICK_ENTITY<"remote"> | PICK_ENTITY<"remote">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: scene
    scene: {
      /**
       * ### apply
       *
       * >
       */
      apply: (service_data?: {
        /**
         * ## entities
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entities": "light.kitchen: \"on\"\nlight.ceiling:\n  state: \"on\"\n  brightness: 80\n"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        entities: Partial<Record<PICK_ENTITY, unknown>> | unknown[];
        /**
         * ## transition
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 0
         * >   max: 300
         * >   unit_of_measurement: seconds
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        transition?: number;
      }) => Promise<void>;
      /**
       * ### create
       *
       * >
       */
      create: (service_data?: {
        /**
         * ## entities
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entities": "light.tv_back_light: \"on\"\nlight.ceiling:\n  state: \"on\"\n  brightness: 200\n"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        entities?: Record<string, unknown> | unknown[];
        /**
         * ## scene_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "scene_id": "all_lights"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        scene_id: string;
        /**
         * ## snapshot_entities
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "snapshot_entities": "- light.ceiling\n- light.kitchen\n"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        snapshot_entities?: PICK_ENTITY | PICK_ENTITY[];
      }) => Promise<void>;
      /**
       * ### delete
       *
       * >
       */
      delete: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - integration: homeassistant
           * >     domain:
           * >       - scene
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"homeassistant", "scene">
            | PICK_FROM_PLATFORM<"homeassistant", "scene">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (
        service_data?: {
          /**
           * ## transition
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 300
           * >   unit_of_measurement: seconds
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          transition?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - scene
           * > ```
           */
          entity_id: PICK_ENTITY<"scene"> | PICK_ENTITY<"scene">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: schedule
    schedule: {
      /**
       * ### get_schedule
       *
       * >
       */
      get_schedule: <T = unknown>(
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - schedule
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<T>;
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
    };
    // # MARK: scheduler
    scheduler: {
      /**
       * ### Add
       *
       * > Create a new schedule entity
       */
      add: (service_data?: {
        /**
         * ## End date
         *
         * > Date until which schedule should be executed
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "end_date": "[\"2021-12-31\"]"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        end_date?: Record<string, unknown> | unknown[];
        /**
         * ## Name
         *
         * > Friendly name for the schedule
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "name": "My schedule"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        name?: string;
        /**
         * ## Repeat Type
         *
         * > Control what happens after the schedule is triggered
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "repeat_type": "\"repeat\""
         * > }
         * > ```
         */
        repeat_type: "repeat" | "single" | "pause";
        /**
         * ## Start date
         *
         * > Date from which schedule should be executed
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "start_date": "[\"2021-01-01\"]"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        start_date?: Record<string, unknown> | unknown[];
        /**
         * ## Timeslots
         *
         * > list of timeslots with their actions and optionally conditions (should be kept the same for all timeslots)
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "timeslots": "[{start: \"12:00\", stop: \"13:00\", actions: [{service: \"light.turn_on\", entity_id: \"light.my_lamp\", service_data: {brightness: 200}}]}]"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        timeslots: Record<string, unknown> | unknown[];
        /**
         * ## Weekdays
         *
         * > Days of the week for which the schedule should be repeated
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "weekdays": "[\"daily\"]"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        weekdays?: Record<string, unknown> | unknown[];
      }) => Promise<void>;
      /**
       * ### Copy
       *
       * > Duplicate a schedule entity
       */
      copy: (service_data?: {
        /**
         * ## Entity
         *
         * > Identifier of the scheduler entity.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entity_id": "switch.schedule_abcdef"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   integration: scheduler
         * >   domain:
         * >     - switch
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id:
          | PICK_FROM_PLATFORM<"scheduler", "switch">
          | PICK_FROM_PLATFORM<"scheduler", "switch">[];
        /**
         * ## Name
         *
         * > Friendly name for the copied schedule
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "name": "My schedule"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        name?: string;
      }) => Promise<void>;
      /**
       * ### Disable All
       *
       * > Disables all schedules
       */
      disable_all: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### Edit
       *
       * > Edit a schedule entity
       */
      edit: (service_data?: {
        /**
         * ## End date
         *
         * > Date until which schedule should be executed
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "end_date": "[\"2021-12-31\"]"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        end_date?: Record<string, unknown> | unknown[];
        /**
         * ## Entity
         *
         * > Identifier of the scheduler entity.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entity_id": "switch.schedule_abcdef"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   integration: scheduler
         * >   domain:
         * >     - switch
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id:
          | PICK_FROM_PLATFORM<"scheduler", "switch">
          | PICK_FROM_PLATFORM<"scheduler", "switch">[];
        /**
         * ## Name
         *
         * > Friendly name for the schedule
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "name": "My schedule"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        name?: string;
        /**
         * ## Repeat Type
         *
         * > Control what happens after the schedule is triggered
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "repeat_type": "\"repeat\""
         * > }
         * > ```
         */
        repeat_type?: "repeat" | "single" | "pause";
        /**
         * ## Start date
         *
         * > Date from which schedule should be executed
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "start_date": "[\"2021-01-01\"]"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        start_date?: Record<string, unknown> | unknown[];
        /**
         * ## Timeslots
         *
         * > list of timeslots with their actions and optionally conditions (should be kept the same for all timeslots)
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "timeslots": "[{start: \"12:00\", stop: \"13:00\", actions: [{service: \"light.turn_on\", entity_id: \"light.my_lamp\", service_data: {brightness: 200}}]}]"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        timeslots?: Record<string, unknown> | unknown[];
        /**
         * ## Weekdays
         *
         * > Days of the week for which the schedule should be repeated
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "weekdays": "[\"daily\"]"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        weekdays?: Record<string, unknown> | unknown[];
      }) => Promise<void>;
      /**
       * ### Enable All
       *
       * > Enables all schedules
       */
      enable_all: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### Remove
       *
       * > Remove a schedule entity
       */
      remove: (service_data: {
        /**
         * ## Entity
         *
         * > Identifier of the scheduler entity.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entity_id": "switch.schedule_abcdef"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   integration: scheduler
         * >   domain:
         * >     - switch
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id:
          | PICK_FROM_PLATFORM<"scheduler", "switch">
          | PICK_FROM_PLATFORM<"scheduler", "switch">[];
      }) => Promise<void>;
      /**
       * ### Run Action
       *
       * > Execute the action of a schedule, optionally at a given time.
       */
      run_action: (service_data?: {
        /**
         * ## Entity
         *
         * > Identifier of the scheduler entity.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entity_id": "switch.schedule_abcdef"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   integration: scheduler
         * >   domain:
         * >     - switch
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id:
          | PICK_FROM_PLATFORM<"scheduler", "switch">
          | PICK_FROM_PLATFORM<"scheduler", "switch">[];
        /**
         * ## Skip Conditions
         *
         * > Whether the conditions of the schedule should be skipped or not
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        skip_conditions?: boolean;
        /**
         * ## Time
         *
         * > Time for which to evaluate the action (only useful for schedules with multiple timeslot)
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "time": "\"12:00\""
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > time: {}
         * > ```
         */
        time?: string;
      }) => Promise<void>;
    };
    // # MARK: script
    script: {
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### toggle
       *
       * >
       */
      toggle: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - script
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_off
       *
       * >
       */
      turn_off: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - script
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - script
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: select
    select: {
      /**
       * ### select_first
       *
       * >
       */
      select_first: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - select
           * > ```
           */
          entity_id: PICK_ENTITY<"select"> | PICK_ENTITY<"select">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### select_last
       *
       * >
       */
      select_last: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - select
           * > ```
           */
          entity_id: PICK_ENTITY<"select"> | PICK_ENTITY<"select">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### select_next
       *
       * >
       */
      select_next: (
        service_data?: {
          /**
           * ## cycle
           *
           * ### Default
           *
           * > ```json
           * > true
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          cycle?: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - select
           * > ```
           */
          entity_id: PICK_ENTITY<"select"> | PICK_ENTITY<"select">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### select_option
       *
       * >
       */
      select_option: (
        service_data: {
          /**
           * ## option
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "option": "\"Item A\""
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   hide_states:
           * >     - unavailable
           * >     - unknown
           * >   multiple: false
           * > ```
           */
          option: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - select
           * > ```
           */
          entity_id: PICK_ENTITY<"select"> | PICK_ENTITY<"select">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### select_previous
       *
       * >
       */
      select_previous: (
        service_data?: {
          /**
           * ## cycle
           *
           * ### Default
           *
           * > ```json
           * > true
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          cycle?: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - select
           * > ```
           */
          entity_id: PICK_ENTITY<"select"> | PICK_ENTITY<"select">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: shelly
    shelly: {
      /**
       * ### get_kvs_value
       *
       * >
       */
      get_kvs_value: <T = unknown>(service_data: {
        /**
         * ## device_id
         *
         * ## Selector
         *
         * > ```yaml
         * > device:
         * >   integration: shelly
         * >   multiple: false
         * > ```
         */
        device_id: TDeviceId;
        /**
         * ## key
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        key: string;
      }) => Promise<T>;
      /**
       * ### set_kvs_value
       *
       * >
       */
      set_kvs_value: (service_data: {
        /**
         * ## device_id
         *
         * ## Selector
         *
         * > ```yaml
         * > device:
         * >   integration: shelly
         * >   multiple: false
         * > ```
         */
        device_id: TDeviceId;
        /**
         * ## key
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        key: string;
        /**
         * ## value
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        value: Record<string, unknown> | unknown[];
      }) => Promise<void>;
    };
    // # MARK: shopping_list
    shopping_list: {
      /**
       * ### add_item
       *
       * >
       */
      add_item: (service_data: {
        /**
         * ## name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "name": "Beer"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        name: string;
      }) => Promise<void>;
      /**
       * ### clear_completed_items
       *
       * >
       */
      clear_completed_items: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### complete_all
       *
       * >
       */
      complete_all: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### complete_item
       *
       * >
       */
      complete_item: (service_data: {
        /**
         * ## name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "name": "Beer"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        name: string;
      }) => Promise<void>;
      /**
       * ### incomplete_all
       *
       * >
       */
      incomplete_all: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### incomplete_item
       *
       * >
       */
      incomplete_item: (service_data: {
        /**
         * ## name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "name": "Beer"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        name: string;
      }) => Promise<void>;
      /**
       * ### remove_item
       *
       * >
       */
      remove_item: (service_data: {
        /**
         * ## name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "name": "Beer"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        name: string;
      }) => Promise<void>;
      /**
       * ### sort
       *
       * >
       */
      sort: (service_data?: {
        /**
         * ## reverse
         *
         * ### Default
         *
         * > ```json
         * > false
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        reverse?: boolean;
      }) => Promise<void>;
    };
    // # MARK: siren
    siren: {
      /**
       * ### toggle
       *
       * >
       */
      toggle: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - siren
           * >     supported_features:
           * >       - 3
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_off
       *
       * >
       */
      turn_off: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - siren
           * >     supported_features:
           * >       - 2
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (
        service_data?: {
          /**
           * ## duration
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "duration": "15"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          duration?: string;
          /**
           * ## tone
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "tone": "fire"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          tone?: string;
          /**
           * ## volume_level
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "volume_level": "0.5"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 1
           * >   step: 0.05
           * >   mode: slider
           * > ```
           */
          volume_level?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - siren
           * >     supported_features:
           * >       - 1
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: sonos
    sonos: {
      /**
       * ### clear_sleep_timer
       *
       * >
       */
      clear_sleep_timer: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - integration: sonos
           * >     domain:
           * >       - media_player
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"sonos", "media_player">
            | PICK_FROM_PLATFORM<"sonos", "media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### get_queue
       *
       * >
       */
      get_queue: <T = unknown>(
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - integration: sonos
           * >     domain:
           * >       - media_player
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"sonos", "media_player">
            | PICK_FROM_PLATFORM<"sonos", "media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<T>;
      /**
       * ### play_queue
       *
       * >
       */
      play_queue: (
        service_data?: {
          /**
           * ## queue_position
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 10000
           * >   mode: box
           * >   step: 1
           * > ```
           */
          queue_position?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - integration: sonos
           * >     domain:
           * >       - media_player
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"sonos", "media_player">
            | PICK_FROM_PLATFORM<"sonos", "media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### remove_from_queue
       *
       * >
       */
      remove_from_queue: (
        service_data?: {
          /**
           * ## queue_position
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 10000
           * >   mode: box
           * >   step: 1
           * > ```
           */
          queue_position?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - integration: sonos
           * >     domain:
           * >       - media_player
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"sonos", "media_player">
            | PICK_FROM_PLATFORM<"sonos", "media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### restore
       *
       * >
       */
      restore: (
        service_data?: {
          /**
           * ## with_group
           *
           * ### Default
           *
           * > ```json
           * > true
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          with_group?: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - integration: sonos
           * >     domain:
           * >       - media_player
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"sonos", "media_player">
            | PICK_FROM_PLATFORM<"sonos", "media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_sleep_timer
       *
       * >
       */
      set_sleep_timer: (
        service_data?: {
          /**
           * ## sleep_time
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 7200
           * >   unit_of_measurement: seconds
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          sleep_time?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - integration: sonos
           * >     domain:
           * >       - media_player
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"sonos", "media_player">
            | PICK_FROM_PLATFORM<"sonos", "media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### snapshot
       *
       * >
       */
      snapshot: (
        service_data?: {
          /**
           * ## with_group
           *
           * ### Default
           *
           * > ```json
           * > true
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          with_group?: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - integration: sonos
           * >     domain:
           * >       - media_player
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"sonos", "media_player">
            | PICK_FROM_PLATFORM<"sonos", "media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### update_alarm
       *
       * >
       */
      update_alarm: (
        service_data?: {
          /**
           * ## alarm_id
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 1
           * >   max: 1440
           * >   mode: box
           * >   step: 1
           * > ```
           */
          alarm_id: number;
          /**
           * ## enabled
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          enabled?: boolean;
          /**
           * ## include_linked_zones
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          include_linked_zones?: boolean;
          /**
           * ## time
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "time": "07:00"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > time: {}
           * > ```
           */
          time?: string;
          /**
           * ## volume
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 1
           * >   step: 0.01
           * >   mode: slider
           * > ```
           */
          volume?: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - integration: sonos
           * >     domain:
           * >       - media_player
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"sonos", "media_player">
            | PICK_FROM_PLATFORM<"sonos", "media_player">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: switch
    switch: {
      /**
       * ### toggle
       *
       * >
       */
      toggle: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - switch
           * > ```
           */
          entity_id: PICK_ENTITY<"switch"> | PICK_ENTITY<"switch">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_off
       *
       * >
       */
      turn_off: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - switch
           * > ```
           */
          entity_id: PICK_ENTITY<"switch"> | PICK_ENTITY<"switch">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - switch
           * > ```
           */
          entity_id: PICK_ENTITY<"switch"> | PICK_ENTITY<"switch">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: synapse
    synapse: {
      /**
       * ### reload
       *
       * > Ask synapse application to re-send the entity list. Sent to all connected by default
       */
      reload: (service_data?: {
        /**
         * ## app
         *
         * > The name of the app to target for the reload. If omitted, targets all apps.
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "app": "home_automation"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * >
         * > ```
         */
        app?: unknown;
      }) => Promise<void>;
    };
    // # MARK: system_log
    system_log: {
      /**
       * ### clear
       *
       * >
       */
      clear: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### write
       *
       * >
       */
      write: (service_data?: {
        /**
         * ## level
         *
         * ### Default
         *
         * > ```json
         * > "error"
         * > ```
         */
        level?: "debug" | "info" | "warning" | "error" | "critical";
        /**
         * ## logger
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "logger": "mycomponent.myplatform"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        logger?: string;
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "Something went wrong"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        message: string;
      }) => Promise<void>;
    };
    // # MARK: template
    template: {
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
    };
    // # MARK: text
    text: {
      /**
       * ### set_value
       *
       * >
       */
      set_value: (
        service_data: {
          /**
           * ## value
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "value": "Hello world!"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          value: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - text
           * > ```
           */
          entity_id: PICK_ENTITY<"text"> | PICK_ENTITY<"text">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: time
    time: {
      /**
       * ### set_value
       *
       * >
       */
      set_value: (
        service_data: {
          /**
           * ## time
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "time": "22:15"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > time: {}
           * > ```
           */
          time: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - time
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: timer
    timer: {
      /**
       * ### cancel
       *
       * >
       */
      cancel: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - timer
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### change
       *
       * >
       */
      change: (
        service_data: {
          /**
           * ## duration
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "duration": "00:01:00, 60 or -60"
           * > }
           * > ```
           *
           * ### Default
           *
           * > ```json
           * > 0
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > duration:
           * >   allow_negative: true
           * >   enable_second: true
           * > ```
           */
          duration: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - timer
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### finish
       *
       * >
       */
      finish: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - timer
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### pause
       *
       * >
       */
      pause: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - timer
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### start
       *
       * >
       */
      start: (
        service_data?: {
          /**
           * ## duration
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "duration": "00:01:00 or 60"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > duration:
           * >   enable_second: true
           * > ```
           */
          duration?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - timer
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: todo
    todo: {
      /**
       * ### add_item
       *
       * >
       */
      add_item: (
        service_data?: {
          /**
           * ## description
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "description": "A more complete description of the to-do item than that provided by the summary."
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          description?: string;
          /**
           * ## due_date
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "due_date": "2023-11-17"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > date: {}
           * > ```
           */
          due_date?: string;
          /**
           * ## due_datetime
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "due_datetime": "2023-11-17 13:30:00"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > datetime: {}
           * > ```
           */
          due_datetime?: string;
          /**
           * ## item
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "item": "Submit income tax return"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          item: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - todo
           * >     supported_features:
           * >       - 1
           * > ```
           */
          entity_id: PICK_ENTITY<"todo"> | PICK_ENTITY<"todo">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### get_items
       *
       * >
       */
      get_items: <T = unknown>(
        service_data?: {
          /**
           * ## status
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "status": "needs_action"
           * > }
           * > ```
           *
           * ### Default
           *
           * > ```json
           * > "needs_action"
           * > ```
           */
          status?: ("needs_action" | "completed")[];
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - todo
           * > ```
           */
          entity_id: PICK_ENTITY<"todo"> | PICK_ENTITY<"todo">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<T>;
      /**
       * ### remove_completed_items
       *
       * >
       */
      remove_completed_items: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - todo
           * >     supported_features:
           * >       - 2
           * > ```
           */
          entity_id: PICK_ENTITY<"todo"> | PICK_ENTITY<"todo">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### remove_item
       *
       * >
       */
      remove_item: (
        service_data: {
          /**
           * ## item
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "item": "Submit income tax return"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          item: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - todo
           * >     supported_features:
           * >       - 2
           * > ```
           */
          entity_id: PICK_ENTITY<"todo"> | PICK_ENTITY<"todo">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### update_item
       *
       * >
       */
      update_item: (
        service_data?: {
          /**
           * ## description
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "description": "A more complete description of the to-do item than that provided by the summary."
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          description?: string;
          /**
           * ## due_date
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "due_date": "2023-11-17"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > date: {}
           * > ```
           */
          due_date?: string;
          /**
           * ## due_datetime
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "due_datetime": "2023-11-17 13:30:00"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > datetime: {}
           * > ```
           */
          due_datetime?: string;
          /**
           * ## item
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "item": "Submit income tax return"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          item: string;
          /**
           * ## rename
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "rename": "Something else"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          rename?: string;
          /**
           * ## status
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "status": "needs_action"
           * > }
           * > ```
           */
          status?: "needs_action" | "completed";
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - todo
           * >     supported_features:
           * >       - 4
           * > ```
           */
          entity_id: PICK_ENTITY<"todo"> | PICK_ENTITY<"todo">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: todoist
    todoist: {
      /**
       * ### new_task
       *
       * >
       */
      new_task: (service_data?: {
        /**
         * ## assignee
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "assignee": "username"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        assignee?: string;
        /**
         * ## content
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "content": "Pick up the mail."
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        content: string;
        /**
         * ## description
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        description?: string;
        /**
         * ## due_date
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "due_date": "2019-10-22"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        due_date?: string;
        /**
         * ## due_date_lang
         */
        due_date_lang?:
          | "da"
          | "de"
          | "en"
          | "es"
          | "fr"
          | "it"
          | "ja"
          | "ko"
          | "nl"
          | "pl"
          | "pt"
          | "ru"
          | "sv"
          | "zh";
        /**
         * ## due_date_string
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "due_date_string": "Tomorrow"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        due_date_string?: string;
        /**
         * ## labels
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "labels": "Chores,Delivieries"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        labels?: string;
        /**
         * ## priority
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 1
         * >   max: 4
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        priority?: number;
        /**
         * ## project
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "project": "Errands"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > "Inbox"
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        project?: string;
        /**
         * ## reminder_date
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "reminder_date": "2019-10-22T10:30:00"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        reminder_date?: string;
        /**
         * ## reminder_date_lang
         */
        reminder_date_lang?:
          | "da"
          | "de"
          | "en"
          | "es"
          | "fr"
          | "it"
          | "ja"
          | "ko"
          | "nl"
          | "pl"
          | "pt"
          | "ru"
          | "sv"
          | "zh";
        /**
         * ## reminder_date_string
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "reminder_date_string": "Tomorrow"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        reminder_date_string?: string;
        /**
         * ## section
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "section": "Deliveries"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        section?: string;
      }) => Promise<void>;
    };
    // # MARK: tts
    tts: {
      /**
       * ### clear_cache
       *
       * >
       */
      clear_cache: (service_data: EmptyObject) => Promise<void>;
      /**
       * ### Say a TTS message with cloud
       *
       * > Say something using text-to-speech on a media player with cloud.
       */
      cloud_say: (service_data?: {
        /**
         * ## cache
         *
         * ### Default
         *
         * > ```json
         * > false
         * > ```
         */
        cache?: boolean;
        /**
         * ## entity_id
         */
        entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
        /**
         * ## language
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "language": "ru"
         * > }
         * > ```
         */
        language?: string;
        /**
         * ## message
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "message": "My name is hanna"
         * > }
         * > ```
         */
        message: string;
        /**
         * ## options
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "options": "platform specific"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object: null
         * > ```
         */
        options?: unknown;
      }) => Promise<void>;
      /**
       * ### speak
       *
       * >
       */
      speak: (
        service_data?: {
          /**
           * ## cache
           *
           * ### Default
           *
           * > ```json
           * > true
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          cache?: boolean;
          /**
           * ## language
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "language": "ru"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          language?: string;
          /**
           * ## media_player_entity_id
           *
           * ## Selector
           *
           * > ```yaml
           * > entity:
           * >   domain:
           * >     - media_player
           * >   reorder: false
           * >   multiple: false
           * > ```
           */
          media_player_entity_id: PICK_ENTITY<"media_player"> | PICK_ENTITY<"media_player">[];
          /**
           * ## message
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "message": "My name is hanna"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          message: string;
          /**
           * ## options
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "options": "platform specific"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > object:
           * >   multiple: false
           * > ```
           */
          options?: Record<string, unknown> | unknown[];
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - tts
           * > ```
           */
          entity_id: PICK_ENTITY<"tts"> | PICK_ENTITY<"tts">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: update
    update: {
      /**
       * ### clear_skipped
       *
       * >
       */
      clear_skipped: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - update
           * > ```
           */
          entity_id: PICK_ENTITY<"update"> | PICK_ENTITY<"update">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### install
       *
       * >
       */
      install: (
        service_data?: {
          /**
           * ## backup
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          backup?: boolean;
          /**
           * ## version
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "version": "1.0.0"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          version?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - update
           * > ```
           */
          entity_id: PICK_ENTITY<"update"> | PICK_ENTITY<"update">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### skip
       *
       * >
       */
      skip: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - update
           * > ```
           */
          entity_id: PICK_ENTITY<"update"> | PICK_ENTITY<"update">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: utility_meter
    utility_meter: {
      /**
       * ### calibrate
       *
       * >
       */
      calibrate: (
        service_data: {
          /**
           * ## value
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "value": "100"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          value: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - sensor
           * >     integration: utility_meter
           * > ```
           */
          entity_id:
            | PICK_FROM_PLATFORM<"utility_meter", "sensor">
            | PICK_FROM_PLATFORM<"utility_meter", "sensor">[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### reset
       *
       * >
       */
      reset: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - select
           * >     integration: utility_meter
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: vacuum
    vacuum: {
      /**
       * ### clean_area
       *
       * >
       */
      clean_area: (
        service_data: {
          /**
           * ## cleaning_area_id
           *
           * ## Selector
           *
           * > ```yaml
           * > area:
           * >   multiple: true
           * >   reorder: true
           * > ```
           */
          cleaning_area_id: TAreaId[];
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - vacuum
           * >     supported_features:
           * >       - 16384
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### clean_spot
       *
       * >
       */
      clean_spot: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - vacuum
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### locate
       *
       * >
       */
      locate: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - vacuum
           * >     supported_features:
           * >       - 512
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### pause
       *
       * >
       */
      pause: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - vacuum
           * >     supported_features:
           * >       - 4
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### return_to_base
       *
       * >
       */
      return_to_base: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - vacuum
           * >     supported_features:
           * >       - 16
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### send_command
       *
       * >
       */
      send_command: (
        service_data?: {
          /**
           * ## command
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "command": "set_dnd_timer"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          command: string;
          /**
           * ## params
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "params": "{ \"key\": \"value\" }"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > object:
           * >   multiple: false
           * > ```
           */
          params?: Record<string, unknown> | unknown[];
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - vacuum
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_fan_speed
       *
       * >
       */
      set_fan_speed: (
        service_data: {
          /**
           * ## fan_speed
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "fan_speed": "low"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   attribute: fan_speed
           * >   multiple: false
           * > ```
           */
          fan_speed: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - vacuum
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### start
       *
       * >
       */
      start: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - vacuum
           * >     supported_features:
           * >       - 8192
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### stop
       *
       * >
       */
      stop: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - vacuum
           * >     supported_features:
           * >       - 8
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: valve
    valve: {
      /**
       * ### close_valve
       *
       * >
       */
      close_valve: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - valve
           * >     supported_features:
           * >       - 2
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### open_valve
       *
       * >
       */
      open_valve: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - valve
           * >     supported_features:
           * >       - 1
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_valve_position
       *
       * >
       */
      set_valve_position: (
        service_data: {
          /**
           * ## position
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 100
           * >   unit_of_measurement: '%'
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          position: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - valve
           * >     supported_features:
           * >       - 4
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### stop_valve
       *
       * >
       */
      stop_valve: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - valve
           * >     supported_features:
           * >       - 8
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### toggle
       *
       * >
       */
      toggle: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - valve
           * >     supported_features:
           * >       - 3
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: wake_on_lan
    wake_on_lan: {
      /**
       * ### send_magic_packet
       *
       * >
       */
      send_magic_packet: (service_data?: {
        /**
         * ## broadcast_address
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "broadcast_address": "192.168.255.255"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        broadcast_address?: string;
        /**
         * ## broadcast_port
         *
         * ### Default
         *
         * > ```json
         * > 9
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 1
         * >   max: 65535
         * >   mode: box
         * >   step: 1
         * > ```
         */
        broadcast_port?: number;
        /**
         * ## mac
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "mac": "aa:bb:cc:dd:ee:ff"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        mac: string;
      }) => Promise<void>;
    };
    // # MARK: water_heater
    water_heater: {
      /**
       * ### set_away_mode
       *
       * >
       */
      set_away_mode: (
        service_data: {
          /**
           * ## away_mode
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          away_mode: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - water_heater
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_operation_mode
       *
       * >
       */
      set_operation_mode: (
        service_data: {
          /**
           * ## operation_mode
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "operation_mode": "eco"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   hide_states:
           * >     - unavailable
           * >     - unknown
           * >   multiple: false
           * > ```
           */
          operation_mode: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - water_heater
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_temperature
       *
       * >
       */
      set_temperature: (
        service_data?: {
          /**
           * ## operation_mode
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "operation_mode": "eco"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > state:
           * >   hide_states:
           * >     - unavailable
           * >     - unknown
           * >   multiple: false
           * > ```
           */
          operation_mode?: string;
          /**
           * ## temperature
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 250
           * >   step: 0.5
           * >   mode: box
           * >   unit_of_measurement: °
           * > ```
           */
          temperature: number;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - water_heater
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_off
       *
       * >
       */
      turn_off: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - water_heater
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### turn_on
       *
       * >
       */
      turn_on: (
        service_data: RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - water_heater
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
    };
    // # MARK: weather
    weather: {
      /**
       * ### get_forecasts
       *
       * >
       */
      get_forecasts: <
        ENTITIES extends PICK_ENTITY<"weather">,
        T = Record<
          ENTITIES,
          {
            forecast: WeatherGetForecasts[];
          }
        >,
      >(
        service_data: {
          /**
           * ## type
           */
          type: "daily" | "hourly" | "twice_daily";
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - weather
           * >     supported_features:
           * >       - 1
           * >       - 2
           * >       - 4
           * > ```
           */
          entity_id: ENTITIES | ENTITIES[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<T>;
    };
    // # MARK: webostv
    webostv: {
      /**
       * ### button
       *
       * >
       */
      button: (service_data: {
        /**
         * ## button
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "button": "LEFT"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        button: string;
        /**
         * ## entity_id
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   integration: webostv
         * >   domain:
         * >     - media_player
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id:
          | PICK_FROM_PLATFORM<"webostv", "media_player">
          | PICK_FROM_PLATFORM<"webostv", "media_player">[];
      }) => Promise<void>;
      /**
       * ### command
       *
       * >
       */
      command: <T = unknown>(service_data?: {
        /**
         * ## command
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "command": "system.launcher/open"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        command: string;
        /**
         * ## entity_id
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   integration: webostv
         * >   domain:
         * >     - media_player
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id:
          | PICK_FROM_PLATFORM<"webostv", "media_player">
          | PICK_FROM_PLATFORM<"webostv", "media_player">[];
        /**
         * ## payload
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "payload": "target: https://www.google.com"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        payload?: Record<string, unknown> | unknown[];
      }) => Promise<T>;
      /**
       * ### select_sound_output
       *
       * >
       */
      select_sound_output: <T = unknown>(service_data: {
        /**
         * ## entity_id
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   integration: webostv
         * >   domain:
         * >     - media_player
         * >   reorder: false
         * >   multiple: false
         * > ```
         */
        entity_id:
          | PICK_FROM_PLATFORM<"webostv", "media_player">
          | PICK_FROM_PLATFORM<"webostv", "media_player">[];
        /**
         * ## sound_output
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "sound_output": "external_speaker"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        sound_output: string;
      }) => Promise<T>;
    };
    // # MARK: zone
    zone: {
      /**
       * ### reload
       *
       * >
       */
      reload: (service_data: EmptyObject) => Promise<void>;
    };
    // # MARK: zwave_js
    zwave_js: {
      /**
       * ### bulk_set_partial_config_parameters
       *
       * >
       */
      bulk_set_partial_config_parameters: (service_data?: {
        /**
         * ## area_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "area_id": "living_room"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > area:
         * >   device:
         * >     - integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        area_id?: TAreaId[];
        /**
         * ## device_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "device_id": "8f4219cfa57e23f6f669c4616c2205e2"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > device:
         * >   filter:
         * >     - integration: zwave_js
         * >   multiple: true
         * > ```
         */
        device_id?: TDeviceId[];
        /**
         * ## endpoint
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "endpoint": "1"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > 0
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        endpoint?: string;
        /**
         * ## entity_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entity_id": "sensor.living_room_temperature"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   filter:
         * >     - integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        entity_id?: PICK_ENTITY | PICK_ENTITY[];
        /**
         * ## parameter
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "parameter": "9"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        parameter: string;
        /**
         * ## value
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "value": "\"0x1\": 1\n\"0x10\": 1\n\"0x20\": 1\n\"0x40\": 1\n"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        value: Record<string, unknown> | unknown[];
      }) => Promise<void>;
      /**
       * ### clear_lock_usercode
       *
       * >
       */
      clear_lock_usercode: (
        service_data: {
          /**
           * ## code_slot
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "code_slot": "1"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          code_slot: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lock
           * >     integration: zwave_js
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### get_lock_usercode
       *
       * >
       */
      get_lock_usercode: <T = unknown>(
        service_data?: {
          /**
           * ## code_slot
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "code_slot": "1"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          code_slot?: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lock
           * >     integration: zwave_js
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<T>;
      /**
       * ### invoke_cc_api
       *
       * >
       */
      invoke_cc_api: (service_data?: {
        /**
         * ## area_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "area_id": "living_room"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > area:
         * >   device:
         * >     - integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        area_id?: TAreaId[];
        /**
         * ## command_class
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "command_class": "132"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        command_class: string;
        /**
         * ## device_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "device_id": "8f4219cfa57e23f6f669c4616c2205e2"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > device:
         * >   filter:
         * >     - integration: zwave_js
         * >   multiple: true
         * > ```
         */
        device_id?: TDeviceId[];
        /**
         * ## endpoint
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "endpoint": "1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        endpoint?: string;
        /**
         * ## entity_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entity_id": "sensor.living_room_temperature"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   filter:
         * >     - integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        entity_id?: PICK_ENTITY | PICK_ENTITY[];
        /**
         * ## method_name
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "method_name": "setInterval"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        method_name: string;
        /**
         * ## parameters
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "parameters": "[1, 1]"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        parameters: Record<string, unknown> | unknown[];
      }) => Promise<void>;
      /**
       * ### multicast_set_value
       *
       * >
       */
      multicast_set_value: (service_data?: {
        /**
         * ## area_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "area_id": "living_room"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > area:
         * >   device:
         * >     - integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        area_id?: TAreaId[];
        /**
         * ## broadcast
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "broadcast": "true"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        broadcast?: boolean;
        /**
         * ## command_class
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "command_class": "117"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        command_class: string;
        /**
         * ## device_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "device_id": "8f4219cfa57e23f6f669c4616c2205e2"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > device:
         * >   filter:
         * >     - integration: zwave_js
         * >   multiple: true
         * > ```
         */
        device_id?: TDeviceId[];
        /**
         * ## endpoint
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "endpoint": "1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        endpoint?: string;
        /**
         * ## entity_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entity_id": "sensor.living_room_temperature"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   filter:
         * >     - integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        entity_id?: PICK_ENTITY | PICK_ENTITY[];
        /**
         * ## options
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        options?: Record<string, unknown> | unknown[];
        /**
         * ## property
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "property": "currentValue"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        property: string;
        /**
         * ## property_key
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "property_key": "1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        property_key?: string;
        /**
         * ## value
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "value": "ffbb99"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        value: Record<string, unknown> | unknown[];
      }) => Promise<void>;
      /**
       * ### ping
       *
       * >
       */
      ping: (service_data?: {
        /**
         * ## area_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "area_id": "living_room"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > area:
         * >   device:
         * >     - integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        area_id?: TAreaId[];
        /**
         * ## device_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "device_id": "8f4219cfa57e23f6f669c4616c2205e2"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > device:
         * >   filter:
         * >     - integration: zwave_js
         * >   multiple: true
         * > ```
         */
        device_id?: TDeviceId[];
        /**
         * ## entity_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entity_id": "sensor.living_room_temperature"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   filter:
         * >     - integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        entity_id?: PICK_ENTITY | PICK_ENTITY[];
      }) => Promise<void>;
      /**
       * ### refresh_notifications
       *
       * >
       */
      refresh_notifications: (service_data?: {
        /**
         * ## area_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "area_id": "living_room"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > area:
         * >   device:
         * >     - integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        area_id?: TAreaId[];
        /**
         * ## device_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "device_id": "8f4219cfa57e23f6f669c4616c2205e2"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > device:
         * >   filter:
         * >     - integration: zwave_js
         * >   multiple: true
         * > ```
         */
        device_id?: TDeviceId[];
        /**
         * ## entity_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entity_id": "sensor.living_room_temperature"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   filter:
         * >     - integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        entity_id?: PICK_ENTITY | PICK_ENTITY[];
        /**
         * ## notification_event
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "notification_event": "1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 1
         * >   max: 255
         * >   mode: box
         * >   step: 1
         * > ```
         */
        notification_event?: number;
        /**
         * ## notification_type
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "notification_type": "1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 1
         * >   max: 22
         * >   mode: box
         * >   step: 1
         * > ```
         */
        notification_type: number;
      }) => Promise<void>;
      /**
       * ### refresh_value
       *
       * >
       */
      refresh_value: (service_data?: {
        /**
         * ## entity_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entity_id": "- sensor.family_room_motion\n- switch.kitchen\n"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        entity_id: PICK_FROM_PLATFORM<"zwave_js"> | PICK_FROM_PLATFORM<"zwave_js">[];
        /**
         * ## refresh_all_values
         *
         * ### Default
         *
         * > ```json
         * > false
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        refresh_all_values?: boolean;
      }) => Promise<void>;
      /**
       * ### reset_meter
       *
       * >
       */
      reset_meter: (service_data?: {
        /**
         * ## area_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "area_id": "living_room"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > area:
         * >   entity:
         * >     - integration: zwave_js
         * >       domain:
         * >         - sensor
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        area_id?: TAreaId[];
        /**
         * ## device_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "device_id": "8f4219cfa57e23f6f669c4616c2205e2"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > device:
         * >   entity:
         * >     - integration: zwave_js
         * >       domain:
         * >         - sensor
         * >   multiple: true
         * > ```
         */
        device_id?: TDeviceId[];
        /**
         * ## entity_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entity_id": "sensor.living_room_temperature"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   filter:
         * >     - integration: zwave_js
         * >       domain:
         * >         - sensor
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        entity_id?: PICK_ENTITY | PICK_ENTITY[];
        /**
         * ## meter_type
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "meter_type": "1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        meter_type?: string;
        /**
         * ## value
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "value": "5"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        value?: string;
      }) => Promise<void>;
      /**
       * ### set_config_parameter
       *
       * >
       */
      set_config_parameter: (service_data?: {
        /**
         * ## area_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "area_id": "living_room"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > area:
         * >   device:
         * >     - integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        area_id?: TAreaId[];
        /**
         * ## bitmask
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        bitmask?: string;
        /**
         * ## device_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "device_id": "8f4219cfa57e23f6f669c4616c2205e2"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > device:
         * >   filter:
         * >     - integration: zwave_js
         * >   multiple: true
         * > ```
         */
        device_id?: TDeviceId[];
        /**
         * ## endpoint
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "endpoint": "1"
         * > }
         * > ```
         *
         * ### Default
         *
         * > ```json
         * > 0
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        endpoint?: string;
        /**
         * ## entity_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entity_id": "sensor.living_room_temperature"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   filter:
         * >     - integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        entity_id?: PICK_ENTITY | PICK_ENTITY[];
        /**
         * ## parameter
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "parameter": "Minimum brightness level"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        parameter: string;
        /**
         * ## value
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "value": "5"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        value: string;
        /**
         * ## value_format
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "value_format": "1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 0
         * >   max: 3
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        value_format?: number;
        /**
         * ## value_size
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "value_size": "1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > number:
         * >   min: 1
         * >   max: 4
         * >   step: 1
         * >   mode: slider
         * > ```
         */
        value_size?: number;
      }) => Promise<void>;
      /**
       * ### set_lock_configuration
       *
       * >
       */
      set_lock_configuration: (
        service_data?: {
          /**
           * ## auto_relock_time
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "auto_relock_time": "1"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 65535
           * >   unit_of_measurement: sec
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          auto_relock_time?: number;
          /**
           * ## block_to_block
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "block_to_block": "true"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          block_to_block?: boolean;
          /**
           * ## hold_and_release_time
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "hold_and_release_time": "1"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 65535
           * >   unit_of_measurement: sec
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          hold_and_release_time?: number;
          /**
           * ## lock_timeout
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "lock_timeout": "1"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > number:
           * >   min: 0
           * >   max: 65535
           * >   unit_of_measurement: sec
           * >   step: 1
           * >   mode: slider
           * > ```
           */
          lock_timeout?: number;
          /**
           * ## operation_type
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "operation_type": "timed"
           * > }
           * > ```
           */
          operation_type: "constant" | "timed";
          /**
           * ## twist_assist
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "twist_assist": "true"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > boolean: {}
           * > ```
           */
          twist_assist?: boolean;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lock
           * >     integration: zwave_js
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_lock_usercode
       *
       * >
       */
      set_lock_usercode: (
        service_data: {
          /**
           * ## code_slot
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "code_slot": "1"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          code_slot: string;
          /**
           * ## usercode
           *
           * ### Example
           *
           * > ```json
           * > {
           * >   "usercode": "1234"
           * > }
           * > ```
           *
           * ## Selector
           *
           * > ```yaml
           * > text:
           * >   multiple: false
           * >   multiline: false
           * > ```
           */
          usercode: string;
        } & RequireAtLeastOne<{
          /**
           * Assisted definition
           * > ```yaml
           * > entity:
           * >   - domain:
           * >       - lock
           * >     integration: zwave_js
           * > ```
           */
          entity_id: never | never[];
          device_id: TDeviceId | TDeviceId[];
          label_id: TLabelId | TLabelId[];
          area_id: TAreaId | TAreaId[];
        }>,
      ) => Promise<void>;
      /**
       * ### set_value
       *
       * >
       */
      set_value: (service_data?: {
        /**
         * ## area_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "area_id": "living_room"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > area:
         * >   device:
         * >     - integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        area_id?: TAreaId[];
        /**
         * ## command_class
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "command_class": "117"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        command_class: string;
        /**
         * ## device_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "device_id": "8f4219cfa57e23f6f669c4616c2205e2"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > device:
         * >   filter:
         * >     - integration: zwave_js
         * >   multiple: true
         * > ```
         */
        device_id?: TDeviceId[];
        /**
         * ## endpoint
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "endpoint": "1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        endpoint?: string;
        /**
         * ## entity_id
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "entity_id": "sensor.living_room_temperature"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > entity:
         * >   filter:
         * >     - integration: zwave_js
         * >   multiple: true
         * >   reorder: false
         * > ```
         */
        entity_id?: PICK_ENTITY | PICK_ENTITY[];
        /**
         * ## options
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        options?: Record<string, unknown> | unknown[];
        /**
         * ## property
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "property": "currentValue"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        property: string;
        /**
         * ## property_key
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "property_key": "1"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > text:
         * >   multiple: false
         * >   multiline: false
         * > ```
         */
        property_key?: string;
        /**
         * ## value
         *
         * ### Example
         *
         * > ```json
         * > {
         * >   "value": "ffbb99"
         * > }
         * > ```
         *
         * ## Selector
         *
         * > ```yaml
         * > object:
         * >   multiple: false
         * > ```
         */
        value: Record<string, unknown> | unknown[];
        /**
         * ## wait_for_result
         *
         * ## Selector
         *
         * > ```yaml
         * > boolean: {}
         * > ```
         */
        wait_for_result?: boolean;
      }) => Promise<void>;
    };
  }
}
